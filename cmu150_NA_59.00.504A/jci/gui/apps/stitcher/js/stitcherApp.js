...
stitcherApp.prototype.appInit = function()
{
	...
	//storing current show name
	this._cachedShowname = null;
...
stitcherApp.prototype._ShowInfoMsgHandler = function(msg)
{
    ...
    if (msg.params.payload.show.name && msg.params.payload.show.name != this._cachedShowname)
    {
        this._cachedShowArt = null;
    }
    ...
    // store new show info
	if(msg.params.payload.show && msg.params.payload.show.name)
	{
		this._cachedShowname = msg.params.payload.show.name;
	}
	log.info("_ShowInfoMsgHandler this._cachedShowname : "+this._cachedShowname);
	log.info("_ShowInfoMsgHandler this._cachedShowArt : "+this._cachedShowArt);
    // update
    if (this._currentContext && this._currentContextTemplate && "NowPlayingOnDemand" === this._currentContext.ctxtId)
    {
        this._updateShowArt(this._cachedShowArt);
...
stitcherApp.prototype._ShowArtMsgHandler = function(msg)
{
    ...
    // store
	if(msg.params.payload.path)
	{
		this._cachedShowArt = msg.params.payload.path;
	}
    
	log.info("Received album art from ShowArt as "+this._cachedShowArt);
    // update
    if (this._currentContext && this._currentContextTemplate && "NowPlayingOnDemand" === this._currentContext.ctxtId)
    {
        this._updateShowArt(this._cachedShowArt);
        ...
        case 'TimedSbn_ErrorCondition' :
            ...
            switch (msg.params.payload.errorID)
            {
                ...
                case 'ConnectionLost' :
                    ...
                    break;
				/*Commented due to Truncation seen in CI-4953
				case 'DeviceDisconnect' :
					errorStringId = 'BTAudioConnectionLost';
					break;*/
            }
