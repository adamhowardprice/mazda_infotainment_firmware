	...
	&& msg.params.payload.favPlayStatus != null && msg.params.payload.favPlayStatus != undefined)
	{
		...
		if(this._currentContext && this._currentContext.ctxtId === "FavoritesRadioTab" && this._currentContextTemplate)
		{
			...
			if(this._cachedFavPlayStatus == true)
			{
				...
				index = this._indexOfFavorite(this._EnumTabRadio,this._cachedActiveFavId);
				...
				log.info("Index to set Focus is: "+(index+1));
				this._currentContextTemplate.list2Ctrl.focussedItem = index+1;
			}
...
favoritesApp.prototype._lazyRequestNaviFavorites = function()
{
    ...
    if(this._currentContextTemplate){
        ...
        else if(this._currentContext.ctxtId === "AddEditFavoritesNavi")
        {
        	if(this._isSdCardConnected)
            {
            	log.info("Requesting APPSDK List ");
                var callBackForList = this._ResponseAddEditNaviFav.bind(this);
                var offset = 0;
                var params = {
                      "StartIndex" : offset,
                      "MaxItems" : 20
...
favoritesApp.prototype._populateFavNaviSelectFav = function ()
{
    ...
    if (!currList.hasDataList())
    {
        ...
        if(this._isSdCardConnected && dataList.items.length > 0 )
		{
            dataList.items[0].disabled = false ;
		...
		else if(dataList.items.length > 0)
		{
            dataList.items[0].disabled = true ;
		...
		{
			log.info("_populateFavNaviSelectFav No list item is present so We are setting SetLoading to false");
			this._currentContextTemplate.list2Ctrl.setLoading(false);
		}
    ...
    else
    {
       // update the new items
        ...
        for (var i=offset; i<offset+count; i++)
        {
            if(list[i-offset].id === 0)
            {
            	 log.info("_populateFavNaviSelectFav ::Home Id Available");
                this._homeIdAvailable = true;
                continue;
            }
    ...
    if(this.isHomeSet)
    {
    	log.info('Setting Home item with Id =0');
   ...
   {
   		log.info('No  Home item with Id =0');
        for(var i = 0;i< dataList.items.length;i++)
       {
...
favoritesApp.prototype._populateFavNaviTab = function ()
{
    ...
    else
    {
       // update the new items
        for (var i=offset; i<offset+count; i++)
        {
            if(list[i-offset].id === 0)
            {
            	log.info("_populateFavNaviTab :: Setting homeId = True");
                this._homeIdAvailable = true;
                //this._favoritesTotalCount = this._favoritesTotalCount + 1;
        ...
        currList.dataList.itemCount = currList.dataList.items.length;
        if(count!=0)
            currList.updateItems(offset+1, (offset+count));
        else
            currList.updateItems(0,currList.dataList.items.length);
        dataList = currList.dataList;
    }
    if(this._homeIdAvailable)
    {
    	log.info("_populateFavNaviTab ::Home Id Available");
        this._cachedNaviFavorites[0] = { id : 0,name: "Home", image : ""};
    ...
    {
    	log.info("_populateFavNaviTab ::Home Id NOT Available");
        for(var i = 0;i< dataList.items.length-2;i++)
        {
...
favoritesApp.prototype._ResponseAddEditNaviFav = function(response)
{
	if(response.msgType === "methodResponse")
	{
        log.info("msgType:methodResponse from Appsdk");
        if (response.serviceName === "navifav" && response.methodName === "GetFavoriteList")
        {
        	var naviFavList = response.params.FavoriteList.favoriteList;
        	var naviFavListCount = response.params.FavoriteList.count ;
        	this._cachedNaviFavoritesCount = naviFavListCount ; 
        	var homeAvailable = false; 
        	for (var i=0; i<naviFavListCount; i++)
            {
                if(naviFavList[i].id === 0)
                {
                	homeAvailable = true;
                	break;
                }
           	...
           	if(homeAvailable)
           	{
           		log.info("Home ID is available");
           		this._cachedNaviFavorites[0] = { id : 0,name: "Home", image : ""};
           	}
           	else
           	{
       		 	log.info("Home ID not available");
           	}
       	 	var list2Ctrl = this._getCurrentContextListCtrl();
   	...
   	{
       		log.info("msgType:methodErrorResponse from Appsdk");
   	}
};
//AppSDK Response Handler for total count
favoritesApp.prototype._GetAppSDKResponseForListCallback = function(response)
{
    ...
    if(response.msgType === "methodResponse"){
        ...
        if (response.serviceName === "navifav" && response.methodName === "GetFavoriteList")
        {
            log.info(" If Navigation Fav receives Error, GUI should show Default Favorites 1.) Add/Edit Radio Favorites 2.) Home");
            this._populateFavNaviTab();
        }
    }
}
        ...
        else if (response.serviceName === "fav" && response.methodName === "GetFavoriteList")
        {
            if (response.params.favList && response.params.favList.arr_fav)
            {
                ...
                if (response.params.return_category === this._EnumGetFavListCategoryRadio)
                {
						...
						{
							this._cachedRadioFavorites = this._createRadioList(response.params.favList.arr_fav); 
						}
						else
						{
...
favoritesApp.prototype._createRadioList = function(list)
{
	var radioList = list;
	for( i=0; i < list.length; i++ )
	{
		if( list[i].type === this._EnumFavTypeFM|| list [i].type === this._EnumFavTypeAM )
		{
			
			var freq = "";
			if(list [i].type === this._EnumFavTypeFM)
			{
				freq = parseFloat(list[i].name);
				freq = freq.toFixed(1);
			}
			else
			{
				freq = list[i].name;
				var index = freq.indexOf('.');
				if(index !== -1)
				{
			...
			if(list[i].stationName !== "") // Station name is present
			{
				log.info("About to set frequency as : "+freq +", name as : "+list[i].stationName+" for item at place: "+i);
				radioList[i].frequency = freq;
				radioList[i].name = list[i].stationName;
			}
			else
			{
				radioList[i].frequency = freq;
				radioList[i].name = freq;
				log.info("About to set frequency as : "+freq +", name as : "+freq+" for item at place: "+i);	
			}
		}
	}
	return radioList;
        ...
        for (var i = 0; i < cached.length; i += 1)
        {
            ...
            if(i === this.idPosition && favoritesTab === this._EnumTabNavi && this.idPosition != null){
                continue; // Skip the addition of Home in item List
					...
					{
						log.info("In _getFavoritesAsListItems Radio tab ");
						var item = this._getItem(cached,i,itemBehavior);
					}
					else
					{
						log.info("In _getFavoritesAsListItems other than Radio tab ");
						var item = 
						{
							appData : cached[i].id,
							text1 : cached[i].name,
...
favoritesApp.prototype.getStationAndRelay = function(stationName,RelayName)
{
    var stationRelay = "";
...
favoritesApp.prototype._getItem = function(cachedList, index,itemBehavior)
{
	var cached = cachedList;
	var item;
	var type = cached[index].type;
	if( framework.localize.getRegion() === framework.localize.REGIONS.Japan )
	{
		if(type === this._EnumFavTypeAM || type === "AM")
        {
			var stationRelayName = ""; 
			//stationRelayName = this._currentContextTemplate.list2Ctrl.getStationAndRelay(cached[index].stationName,cached[index].relayName);
			stationRelayName = this.getStationAndRelay(cached[index].stationName,cached[index].relayName);
			log.info(" In _getItem StationRelayName: "+stationRelayName);
			item = 
			{
				appData : cached[index].id,
				text1 : cached[index].frequency ,
				label1: "KHz",
				label2: stationRelayName,
				itemStyle : "style38", 
				image1 : null,
				hasCaret : false,
				itemBehavior : itemBehavior,
...
favoritesApp.prototype._checkSetHomeStatus = function(){
    //Checking for NaviFavoritesItem Count
     log.info("#Fav_checkSetHomeStatus:this._cachedNaviFavoritesCount::"+this._cachedNaviFavoritesCount);
    if(this._cachedNaviFavoritesCount === 0)
    {
         log.info("::In FavoritesNaviTab this._cachedNaviFavoritesCount is 0 resetting flag");
         this.idPosition = null ;
         this.isHomeSet = false ;
    }
    else
    {
        log.info("::In FavoritesNaviTab this._cachedNaviFavoritesCount is not 0 checking for home set");

        this._homeIdAvailable = false;
        for(var i = 0; i < this._cachedNaviFavorites.length; i++){
            if(this._cachedNaviFavorites[i].id === undefined){
                log.info("In FavoritesNaviTab this._cachedNaviFavorites[i].id is undefined "+i);
            }else{
                log.info("In FavoritesNaviTab this._cachedNaviFavorites[i].id is defined"+i);
            }
            if(this._cachedNaviFavorites[i].id === 0 && this._cachedNaviFavorites[i].id !== undefined){
                this.idPosition = i; 
                log.info("Home is set::enable the Delete Home Button::IdPosition is::"+this.idPosition +"   "+i);
                this.isHomeSet = true ;
                break;
            }else{
                this.idPosition = null ;
                log.info("Home is not set::disable the Delete Home Button::idPosition is"+this.idPosition +"   "+i);
                this.isHomeSet = false ;
            }
        }
    }
     ...
     else
     {
         ...
         for (var i = 0; i < newData.items.length; i += 1)
         {
             ...
             else
             {
				 ...
				 oldData.items[i].itemStyle = newData.items[i].itemStyle;
				if( framework.localize.getRegion() === framework.localize.REGIONS.Japan )
				{
					if (type === this._EnumFavTypeAM || type === "AM")
					{
						oldData.items[i].label1 = newData.items[i].label1;
						oldData.items[i].label2 = newData.items[i].label2;
					}
					else if(type === this._EnumFavTypeFM || type === "FM")
					{
						oldData.items[i].label1 = newData.items[i].label1;
    ...
    if (this._currentContext && this._currentContextTemplate)
    {
        switch (this._currentContext.ctxtId)
        {
			...
			//case "SelectPosition":
                ...
                break;
            case "SelectPosition":
                this._disableAllFavoriteListItems(); //RTC Issue:4824
                this._currentContextTemplate.list2Ctrl.setReorderAtSpeed(framework.common.getAtSpeedValue());
                break; 
            case "QwertyEntry":
...
favoritesApp.prototype._disableAllFavoriteListItems = function()
{
    ...
    if (list2Ctrl)
    {
		...
		if (this._currentContext && this._currentContext.ctxtId &&this._currentContext.ctxtId ==="SelectPosition")
		{
			
			if(framework.common.getAtSpeedValue())
			{
				list2Ctrl.dataList.items[list2Ctrl.focussedItem].disabled = false;
				list2Ctrl.dataList.items[list2Ctrl.focussedItem].image1 = imagePath[0]+"_En.png" ;
			}
		}
...
favoritesApp.prototype._naviFavoritesLongPress = function(controlRef, appData, params)
{
    ...
    }else
	{
		...
		{
            pos = this._indexOfFavorite(this._EnumTabNavi, fav.id);
			log.info("In _naviFavoritesLongPress this._homeIdAvailable "+ this._homeIdAvailable);
			if(this._homeIdAvailable) //SW00172883
			{
				pos = pos -1;
			}
        }
        if (fav)
        {
			...
			log.info("NaviFavorite:: SelectFavoriteListLong (Long Press) event send to mmui"); 
			framework.sendEventToMmui(this.uiaId, "SelectFavoriteListLong", eventData);
		}
   }
};
...
favoritesApp.prototype._populateAddEditNaviFavoritesItemList = function(list2Ctrl,data){
    ...
    if(this._currentContext && this._currentContext.ctxtId === "AddEditFavoritesNavi")
    {
		...
		log.info("speed : "+framework.common.getAtSpeedValue());
		log.info("this.isHomeSet : "+this.isHomeSet);
		if(framework.common.getAtSpeedValue())
		{
		...
		{/*167719*/
			if(this.isHomeSet)
			{
				log.info("######################### Home is Set, Enable 'Delete Home' Button "+this.isHomeSet);
				data.items[6].disabled = false ;
			}
			else
			{
				log.info("######################### Home is NOT Set, Disable 'Delete Home' Button "+this.isHomeSet);
				data.items[6].disabled = true ;	
			}
			data.items[7].disabled = false ;
        }
    ...
    else if(this._currentContext && this._currentContext.ctxtId === "EditFavoritesNavi")
    {
        ...
        if(this._isUpdateReplaceNaviList && this._currentContext.ctxtId === "EditFavoritesNavi")
        {
              ...
              this._checkSetHomeStatus();
              log.info("#Fav:AddEditFavoritesNavi Context:: populateAddEditNaviFavoritesItemList after calling this._checkSetHomeStatus()::IsHomeSet flag is::"+this.isHomeSet);
			  log.debug("speed : "+framework.common.getAtSpeedValue());
				log.info("this.isHomeSet : "+this.isHomeSet);
			  	if(framework.common.getAtSpeedValue())
				{
					data.items[6].disabled = true ;
...
favoritesApp.prototype._populateUnSetHomeList = function(){
	...
	log.debug("speed : "+framework.common.getAtSpeedValue());
	log.debug("this.isHomeSet : "+this.isHomeSet);
	if(framework.common.getAtSpeedValue())
	{
		currentList.dataList.items[0].disabled = true ;
		currentList.dataList.items[1].disabled = true ;
	}
	else
	{
		currentList.dataList.items[0].disabled = false ;
		/*167719*/
		if(this.isHomeSet)
		{	
			currentList.dataList.items[1].disabled = false ;
