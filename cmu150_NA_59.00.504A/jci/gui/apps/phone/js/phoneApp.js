...
{ 
    ...
    {          
		...
		log.info(" callNumberCheck : "+this._callNumberCheck+" activeCallPhoneNumber : "+this._activeCallPhoneNumber);
		// null check is there for long ACCON case
		if((this._callNumberCheck != this._activeCallPhoneNumber) || (this._callNumberCheck == null && this._activeCallPhoneNumber == null)) 
		{
			log.info(" calling this._populateActiveCallData ");
            if (this._timeout != null)
            {
                clearTimeout(this._timeout);
                this._timeout = null;
            }
            this._timeout = self.setTimeout(phoneApp.prototype._setTimer.bind(this), 1000);
...
phoneApp.prototype._ActiveCallInfoMsgHandler = function(msg)
{
                            ...
                            && msg.params.payload.activeCallInfoMsg.cmuorphone && msg.params.payload.activeCallInfoMsg.mute)
    {
        ...
        if (this._currentContext && this._currentContextTemplate)
        {
            ...
            {                
                case "ActiveCall":
					log.info("Active Call");
					if(this._activeCallPhoneNumber != this._checkNumber)
					{
						log.info("Populating ActiveCallData from ActiveCallInfoMsgHandler checkNumber : "+this._checkNumber);
						this._populateActiveCallData();
					}
                    if (!this._contactsDbOpenStatus)
                    {
        ...
        if (this._currentContext && this._currentContextTemplate)
        {
            ...
            {                
                ...
                case "ActiveCallMerged": 
					log.info("Updating Active call1 and Call2 data ");
					this._populateActiveCall1Data();
					this._populateActiveCall2Data();
                    if (!this._contactsDbOpenStatus)
                    {
...
{    
    ...
    {        
        // set default values
		this._checkNumber = this._activeCallPhoneNumber;
        log.debug("this._checkNumber"+ this._checkNumber);
        var ctrlData =
        {
            "ctrlStyle" : "OneCall",
    ...
    if (this._currentContext && this._currentContextTemplate)
    {
        ...
        {            
            case "IncomingCall":                             
               ...
               if(this.contactName)
                {
					this._callNumberCheck = this._cachedPhoneNumber;
                    ctrlData = {
                        "contentStyle" : "style04",
...
phoneApp.prototype._setTimer = function()
{
