...
pandoraApp.prototype._TrackInfoMsgHandler = function(msg)
{
	...
	log.info("TrackInfoMsgHandler allowskip : "+this._cachedTrackInfo.allowSkip+" allowbookmark : "+this._cachedTrackInfo.allowBookmark);
    ...
    if (msg.params.payload.errorReceived)
    {
        switch (msg.params.payload.errorReceived)
        {
            ...
            case "NoStations":
			    ...
			    break;
			case "BTAudioDisconnected" :
				errStringId = "BluetoothAudioLost";
				break;
        }
        framework.common.startTimedSbn(this.uiaId, 'Pandora_Error_SBN', 'errorNotification', {
            sbnStyle : 'Style02',
...
pandoraApp.prototype._setRecoverableError = function ()
{
    ...
    tmpltUmp.setButtonDisabled("bookmark", this._cacheRecoverableErrorState);
    
    tmpltUmp.setButtonDisabled("skip", this._cacheRecoverableErrorState);
	
	if(this._cachedTrackInfo)
	{
		log.info("_setRecoverableError allowSkip : "+this._cachedTrackInfo.allowSkip+" allowBookmark : " +this._cachedTrackInfo.allowBookmark+" Allow Rating "+this._cachedTrackInfo.allowRating);
		
		if(this._cachedTrackInfo.allowSkip == false)
		{
			tmpltUmp.setButtonDisabled("skip", true);
		}
		if(this._cachedTrackInfo.allowBookmark == false)
		{
			tmpltUmp.setButtonDisabled("bookmark", true);
		}
    ...
    if (this._cacheRecoverableErrorState == true)
    {
        tmpltUmp.setButtonDisabled("thumbsdown", this._cacheRecoverableErrorState);
        tmpltUmp.setButtonDisabled("thumbsup", this._cacheRecoverableErrorState);
...
pandoraApp.prototype._updateNowPlayingCtrl = function(station)
{
	...
	if(station)
	{
		log.info("_updateNowPlayingCtrl allowSkip : "+station.allowSkip +" allowbookmark : "+station.allowBookmark+"AlloRating "+station.allowRating);
		if(station.allowSkip == false)
		{
			currentNowPlayingCtrl.umpCtrl.setButtonDisabled("skip", true);
		}
		if(station.allowBookmark == false)
		{
			currentNowPlayingCtrl.umpCtrl.setButtonDisabled("bookmark",true);
		}
		if(station.allowRating == false)
		{
			currentNowPlayingCtrl.umpCtrl.setButtonDisabled("thumbsdown",true);
			currentNowPlayingCtrl.umpCtrl.setButtonDisabled("thumbsup",true);
		}
	...
	if(station.type == "Ad")
	{
		currentNowPlayingCtrl.umpCtrl.setButtonDisabled("stationlist",true);
	}
	else if((this._playerState == "buffering") && (station.type != "Ad"))
	{
		currentNowPlayingCtrl.umpCtrl.setButtonDisabled("stationlist",false);
	}
...
pandoraApp.prototype._updateTrackRating = function()
{
    ...
    if (null != this._cachedTrackRating)
    {
		...
		if (this._cachedTrackInfo && this._cachedTrackInfo.allowRating == false)
        {
            currentNowPlayingCtrl.umpCtrl.setButtonDisabled("thumbsup",true);
            currentNowPlayingCtrl.umpCtrl.setButtonDisabled("thumbsdown",true);
        }
        // determine disabled state of the buttons
        if (this._cachedTrackInfo && (this._cachedActiveStationType == "Shared" || this._cacheRecoverableErrorState == true))
        {
            currentNowPlayingCtrl.umpCtrl.setButtonDisabled("thumbsup",true);
            currentNowPlayingCtrl.umpCtrl.setButtonDisabled("thumbsdown",true);
        }
