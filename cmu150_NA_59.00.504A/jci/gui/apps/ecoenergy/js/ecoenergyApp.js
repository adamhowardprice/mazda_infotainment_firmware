/*
 ...
 v0.15(26-April-2015)Regular local unit of fuel monitor is Km/L, however it was displayed as L/100km last 2 times. [SW00168649]
...
ecoenergyApp.prototype.appInit = function()
{
   ...
   this._cached_iEloopEnergy=          {                               //used to cache the iEloopEnergy data for Effectiveness context
    ...
    this._cachedResetToggleStatus  = 0;                               //used to cache the status of resettoggle button
...
{
	log.info('_initialHEVGraphData called');
...
{
	log.info('_newHEVBarData called');
...
ecoenergyApp.prototype._updateEvDistance = function()
{
	 ...
	 {
		//for "zh_TW" "zh_CN" "cn_HK" as per rtc 3929
		var currLang = framework.localize.getCurrLang();
		if((this._unitEFlow  == "Km" || this._cachedEvDriveDistance.unitId == 'km') && (currLang == "zh_TW" || currLang == "zh_CN" || currLang == "cn_HK"))
		{
		this._cachedEvDriveDistance.unitId = "KmDriveMonitorJP";
		}
		else
		{
			if(this._cachedEvDriveDistance.unitId == "KmDriveMonitorJP" && (currLang != "zh_TW" && currLang != "zh_CN" && currLang != "cn_HK"))
			{
				this._cachedEvDriveDistance.unitId = 'km';
			}
		}
...
ecoenergyApp.prototype._updateOneDriveDistance = function()
{
	 if (this._currentContextTemplate && (this._currentContext.ctxtId    ===    "HEVFuelConsumption" || this._currentContext.ctxtId    ===    "EndingHEV"))
	 {
		//For "zh_TW" "zh_CN" "cn_HK" as per rtc 3929
		var currLang = framework.localize.getCurrLang();
		if((this._UnitConsumption == "Km" || this._cachedOneDriveDistance.unitId == 'km') && (currLang == "zh_TW" || currLang == "zh_CN" || currLang == "cn_HK"))
		{
		this._cachedOneDriveDistance.unitId = "KmDriveMonitorJP";
		}
		else
		{
			if(this._cachedOneDriveDistance.unitId == "KmDriveMonitorJP" && (currLang != "zh_TW" && currLang != "zh_CN" && currLang != "cn_HK"))
			{
				this._cachedOneDriveDistance.unitId = 'km';
			}
		}
	        // Update the control to update the EV drive distance 
...
ecoenergyApp.prototype._controlStatusContextIn = function(context)
{
    ...
    log.info("ECOENERGYGUI: ControlStatusContextIn() called...");
...
ecoenergyApp.prototype._controlStatusReady = function()
{
     ...
     */
    log.info("_ecoenergyReady() called...");
...
{
	log.info('_ecoFlowContextIn called');
	if(this._isUmpPanelOpen)
    {
        this._isEcoContext = true; 
    }
...
{
    log.info("ECOENERGYGUI: _EffectivenessContextIn() called..");
...
{
    log.info("_ecoSettingContextIn() called...");
    //cache the info of "ending screen" and "reset" toggle button status. 
     if(msg && msg.params && msg.params.payload && msg.params.payload.settingsEnableDisable && msg.params.payload.endingScreen )
     {
         ...
         this._readSyncStatus = msg.params.payload.sync;   		 //Cache the status of reset toggle button.
	log.info("readSyncStatus :: "+this._readSyncStatus);
         
         if (  this._readSyncStatus != 0)
         {
           this._cachedResetToggleStatus = this._readSyncStatus;
         }      
         this._cachedSettingsEnableDisableStatus = msg.params.payload.settingsEnableDisable;        //Cache the data of ending screen support
         log.info("Ending screen toggle status :: "+this._cachedEndingScreenToggleStatus +"Reset toggle status :: "+this._cachedResetToggleStatus +" Ending screen support :: "+this._cachedSettingsEnableDisableStatus);
    }
		if(this._feature=="HEV")
         {
...
{
     log.info("_ecoResetRetryReady  called..." );
     
};

ecoenergyApp.prototype._ecoResetRetryContextIn = function()
{
    log.info("_ecoResetRetryContextIn  called..." );
    this._isResetFinished = false;
    if(this._feature=="HEV")
    {
...
{
	log.info("_ecoResetConfirmContextIn  called..." );
    this._isResetFinished = false;
    if(this._feature=="HEV")
    {
       this._contextTable['ResetRetry'].sbNameId = "driveMonitor";
...
{
	log.info("_ecoResetConfirmReady  called..." );
};
...
{
     log.info("_ecoSettingRetryContextIn  called..." );
     if(this._feature=="HEV")
     {
...
{
     log.info("_ecoSettingsRetryReady  called..." );
...
{
    log.info("contextInCbEffectivenessEnding() called...");
...
{
    log.info("_fuelConsumptionEndingContextIn() called...");
     if(msg && msg.params && msg.params.payload && msg.params.payload.celebrationCriteria)
     {
         if(msg.params.payload.celebrationCriteria)
        {
	 ...
	 //log.info("msg payload "+JSON.stringify(msg));
	if(msg && msg.params && msg.params.payload)
	{
		this._globalUnit = msg.params.payload.fuelEconomyBarGraphUnit;
	}
...
ecoenergyApp.prototype._setIEloopDataMessageHandler = function(msg)
{
    ...
    if (msg && msg.params && msg.params.payload && msg.params.payload.iELoop_CntrlStatus)// Null Check
    {
        ...
        log.info("setIEloopDataMessageHandler() called ");
        // Make sure we're active (all contexts)
...
{    
    ...
    if (msg && msg.params && msg.params.payload && msg.params.payload.iStop_StatCntrlStatus)// Null Check
    {
        ...
        log.info("setIStopStatusMessageHandler() called .... Engine = "+this._cached_iStopStatusObj.engineStatus
...
ecoenergyApp.prototype._setIStopErrorMessageHandler = function(msg)
{
    ...
    {
        log.info("setIStopErrorMessageHandler() called: "
...
ecoenergyApp.prototype._setIStopTotalMessageHandler = function(msg)
{
    ...
    {// Null Check
        ...
        log.info("setIStopTotalMessageHandler() called: "
...
ecoenergyApp.prototype._fuelConsumptionReady = function()
{
    ...
    if (this._currentContext && this._currentContextTemplate && this._currentContext.ctxtId === "FuelConsumption")
    {
        ...
        obj = this._saveUnitInformation(this._globalUnit, obj);
        this._currentContextTemplate.fuelConsumptionCtrl.setUnitInformation(obj);
		this._globalUnitBackup = this._globalUnit;
...
{
    log.info("_fuelConsumptionContextIn() called...");
...
{
	 log.info("_HEVfuelConsumptionReady() called...");
...
{
	 log.info("_HEVfuelConsumptionContextIn() called...");
	 if(this._isUmpPanelOpen)
	 {
	        this._isEcoContext = true; 
	 }
...
{
	 log.info("_HEVfuelConsumptionReadyEnding() called...");
	 this._currentControl = this._currentContextTemplate.fuelConsumptionCtrl;
...
{
	 log.info("_HEVfuelConsumptionContextInEnding() called...");
	//this._cumulativeFuelConfig =  this._saveUnitInformation(this._globalUnit, this._cumulativeFuelConfig);
    //this._currentFuelConfig =  this._saveUnitInformation(this._globalUnit, this._currentFuelConfig);
 ...
 */
    log.info("CurrDrvEcoPerInstBarGraphMessageHandler() called");
...
ecoenergyApp.prototype._cumFuelResetEcoGraphDataMessageHandler = function(msg)
{
     ...
     * (This data would normally come from MMUI)
     */
    log.info("cumFuelResetEcoGraphDataMessageHandler() called");
...
ecoenergyApp.prototype._currAvgDriveFuelEcoMessageHandler = function(msg)
{
    ...
    if (this._currentContext && this._currentContextTemplate)
    {
        ...
        {
            log.info("We are not in ControlStatus or FuelConsumption context, so not populating the data");
        }
    }
};
...
ecoenergyApp.prototype._newCurrFuelResetEcoGraphDataMessageHandler = function(msg)
{
    ...
    log.info("newCurrFuelResetEcoGraphDataMessageHandler() called: currentBarValue = " 
...
ecoenergyApp.prototype._setiEloopEnergyMsgHandler = function(msg)
{
    ...
    {// Null Check
        ...
        log.info("setiEloopEnergyMsgHandler() called");
        // Make sure we're active (all contexts)
        if (this._currentContext && this._currentContextTemplate
                && (this._currentContext.ctxtId    ===    "Effectiveness" || this._currentContext.ctxtId    ===    "EndingEffectiveness"))
        {
...
ecoenergyApp.prototype._setiStopTimeMsgHandler = function(msg) {
    ...
    {// Null Check{
        ...
        log.info("setiStopTimeMsgHandler() called");
        // Make sure we're active (all contexts)
        if (this._currentContext && this._currentContextTemplate
                && (this._currentContext.ctxtId    ===    "Effectiveness" || this._currentContext.ctxtId    ===    "EndingEffectiveness"))
        {
...
ecoenergyApp.prototype._setiEloopEffectiveRateMsgHandler = function(msg)
{
    ...
    {// Null Check
        ...
        log.info("setiEloopEffectiveRateMsgHandler() called iEloopEffectiveRate = "+
...
ecoenergyApp.prototype._setiStopEffectiveRateMsgHandler = function(msg)
{
    ...
    {// Null Check
        ...
        else
        {
            ...
            log.info("setiStopEffectiveRateMsgHandler() called _cached_iStopEffectiveRate = "+
...
{    /*
     ...
     * (This data would normally come from MMUI)
     */
    log.info("setTreeDataMsgHandler() called");
    // Make sure the received msg is not null
    if (msg && msg.params && msg.params.payload)
    {
        // Cache a copy of the passed-in data
...
{
    log.info("_setBoostRangeHandler() called");
    // Make sure the received msg is not null
    if (msg && msg.params && msg.params.payload)
    {
            ...
            switch(appData)
            {
                ...
                case "ResetTripCAFE":
                     switch(params.additionalData)
                        {
                            case 1:
                                this._cachedResetToggleStatus = 2;
                    ...
                    break;
                default:
                    log.info("In inner switch default case ");
                    break;
            }
};
...
{
   log.info("_dialogSelectCallback  called..." +appData);
        ...
        switch (appData)
        {
            case "Retry":
	            if(this._cachedResetToggleStatus === 1)
	    		{
	           	...
	           	log.info("[_dialogCtrlSettingRetryClickCallback] about to send toggel status :: "+this._cachedResetToggleStatus);
...
{
 log.info("_dialogCtrlLoadingClickCallback  called..." +appData);
 if(this._currentContext.ctxtId)
    {
        switch (appData)
        {           
...
ecoenergyApp.prototype._setFuelEfficiencyMessageHandler = function(msg)
{
    ...
    if (msg && msg.params && msg.params.payload)
    {// Null Check
        if(this._globalUnitBackup && (this._globalUnitBackup != this._globalUnit))
        {
            var obj = new Object();
            obj = this._saveUnitInformation(this._globalUnit, obj);
            this._currentContextTemplate.fuelConsumptionCtrl.setUnitInformation(obj);
            this._updateCurrentCumulativeFuelGraph(this._cached_fuelEfficiencyData.fuelEfficiency, this._globalUnitRange);
            this._currentContextTemplate.fuelConsumptionCtrl._setCFERGraphBarHeights();
            this._globalUnitBackup = this._globalUnit;
        }
	...
	this._cached_currentCumFuelUnit = this._getUnitId(msg.params.payload.CurrentAvgFuelEco.CurdrvAvlFuel_unit);/*SCR SW00168649*/
        //Previous line : this._cached_currentCumFuelUnit = this._getUnitId(this._globalUnit);
        // Make sure we're active (all contexts)
        if (this._currentContext && this._currentContextTemplate
                && (this._currentContext.ctxtId    ===    "FuelConsumption" || this._currentContext.ctxtId    ===    "EndingFuelConsumption"  || this._currentContext.ctxtId    ===    "HEVFuelConsumption"))
        {
...
ecoenergyApp.prototype._setCurrEquippedFeatureMessageHandler = function(msg)
{
     ...
     */
        if (msg && msg.params && msg.params.payload)
        {// Null Check
            log.info("setCurrEquippedFeatureMessageHandler() called: "
            ...
            {
                case "ISTOP_IELOOP":
                	log.info("ECOENERGYGUI: CurrFeature value is iEloopiStop");
                    this._equippedCtrlStyle = "iEloopiStop";
                    this._contextTable['FuelConsumption'].controlProperties.FuelConsumptionCtrl.umpButtonConfig = this._umpButtonConfigFuel;
                    break;
                case "IELOOP":
                ...
                case "NONE":
                    ...
                    log.info("ECOENERGYGUI: CurrFeature value is None");
                    this._contextTable['FuelConsumption'].controlProperties.FuelConsumptionCtrl.umpButtonConfig = this._umpButtonConfigNoSwitch; 
                    break;
                default:
					this._equippedCtrlStyle = "iEloopiStop";
...
ecoenergyApp.prototype._setSyncStatusAndTripA = function(msg)
{
    ...
    if(msg &&  msg.params && msg.params.payload)
    {
		log.info("##### : this._cachedResetToggleStatus :"+this._cachedResetToggleStatus);
...
ecoenergyApp.prototype._AtSpeedMsgHandler = function(msg) 
{
    ...
    if (this._currentContextTemplate && this._currentContext && this._currentContext.ctxtId === "Settings")
    {
        log.info("[AtSpeedMsgHandler] :: about to populatelist with atspeed condtn :: "+this._atSpeedVal);
        this._populateSettingList();
    }
};
...
ecoenergyApp.prototype._NoSpeedMsgHandler = function(msg)
{
    ...
    if (this._currentContextTemplate && this._currentContext && this._currentContext.ctxtId === "Settings")
    {
        log.info("[NoSpeedMsgHandler] :: about to populatelist with atspeed condtn :: "+this._atSpeedVal);
        this._populateSettingList();
...
ecoenergyApp.prototype._resetTripAHandler = function(msg)
{
	...
	if(msg.params.payload.IgnSts)
    {
		...
		log.info("##### : this._cachedResetTripAStatus " + this._cachedResetTripAStatus);
    }
	else
	{
...
ecoenergyApp.prototype._saveUnitInformation = function(unit, obj)
{
	...
	else if(this._cachedVehicleType == "SETTINGS_VehicleModelType_J78A")
	{
		switch(unit)
		{
			case "KMPERLIT":
	...
	else if((this._cachedVehicleType == "SETTINGS_VehicleModelType_J03A") || (this._cachedVehicleType == "SETTINGS_VehicleModelType_J03K")||(this._cachedVehicleType == "SETTINGS_VehicleModelType_J03E") || (this._cachedVehicleType == "SETTINGS_VehicleModelType_J03J"))
	{
		switch(unit)
		{
			case "KMPERLIT":
...
ecoenergyApp.prototype._populateSettingList = function()
{
    ...
    log.info("[populateSettingList] Ending screen toggle status :: "+this._cachedEndingScreenToggleStatus +"Reset toggle status :: "+this._cachedResetToggleStatus +" Ending screen support :: "+this._cachedSettingsEnableDisableStatus);
	...
	else if(this._cachedResetToggleStatus === 2)
	{
    ...
    {
    	 if(this._cachedResetToggleStatus === 0)
	     {
	        dataListItems[1].disabled = true;
	     }
	     else
   ...
   //set datalist on listcontrol
	...
	log.info("#####:_populateSettingList :: this._ecoenergySettingList.items[0].disabled :"+this._ecoenergySettingList.items[0].disabled);
	log.info("#####:_populateSettingList :: this._ecoenergySettingList.items[1].disabled :"+this._ecoenergySettingList.items[1].disabled);
};
