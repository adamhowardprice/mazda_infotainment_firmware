...
{
    log.debug("ENTER _preparingDisplayed, this._preparingStuckTime:" + this._preparingStuckTime);
...
{
    var params = {payload:{}};
    log.warn("ENTER : _preparingStuckTimedout");
    this._preparingStuckTimeoutId = null;
...
{
    log.debug("ENTER activeContextReady, mediaSource:[" + this._cachedMediaSource + "]");
    this._updateSplitScreen();
};

emnaviApp.prototype._ActiveCtxtInFunction = function() 
{
    log.info("ENTER activeContextIn, mediaSource:[" + this._cachedMediaSource + "]" + "screen type:[" + this._currentContext.params.payload.ScreenType + "]");
...
emnaviApp.prototype._ActiveCtxtNoLongerDisplayed = function()
{
	...
	this._contextTable.Active.controlProperties.NowPlayingInfoCtrl.visible = false;
};
...
{
    log.debug('sending IntentNowPlayingScreen');
...
{
    log.info("ENTER _MetaDataListHandler, for source: [" + this._cachedMediaSource + "]"+ "Name : [" + msg.params.payload.Name + "]" + "CommonLine : [" + msg.params.payload.CommonLine + "]" + "Track : [" + msg.params.payload.Track + "]" + "Artist : [" + msg.params.payload.Artist + "]" + "Album : [" + msg.params.payload.Album + "]");
...
emnaviApp.prototype._MetaDataSourceHandler = function(msg) 
{
    ...
    if(this._currentContext && this._currentContextTemplate && this._currentContext.ctxtId == "Active") 
    {
		...
		this._cachedMetaData.Name =""; //rakesh
    ...
    else 
    {
        ...
        log.info("Context not active, source name will not be updated so clearing all cached data..");
...
emnaviApp.prototype._updateSplitScreen = function() {
    ...
    log.info("ENTER _updateSplitScreen, for source: [" + this._cachedMediaSource + "]" +  "Name: [" + this._cachedMetaData.Name +  "]" + "Song/Track/Name: [" + this._cachedMetaData.Track + "]" + "CommonLine: [" + this._cachedMetaData.CommonLine + "]" +  "Artist: [" + this._cachedMetaData.Artist + "]" +  "Album: [" + this._cachedMetaData.Album + "]");
    ...
    if((this._cachedMediaSource == "FMRADIO" || this._cachedMediaSource == "AMRADIO") && this._cachedMetaData.CommonLine !== null ) {
        var metadata = null,
			stationName = null,
		    relay = null, 
			nickname = null;
        ...
        if(freq)
        {
			if(this._cachedMediaSource == "FMRADIO")
		    {     
				metadata = nowPlayingInfoCtrl.getFrequencyAndUnitsText(freq, "MHz");
			}
			else
			{
				metadata = nowPlayingInfoCtrl.getFrequencyAndUnitsText(freq, "kHz");
			}
        }
        else
        {
			log.warn("Since 0 frequency is received in FM radio so not displaying freq on UI");
			metadata = "";
        }
        nowPlayingInfoCtrl.setGenericText(metadata);	
		
		if (this._cachedMetaData.Name)
        {
			stationName = this._cachedMetaData.Name;
        }
		if(!stationName)
        {
            stationName = "";
        }
		relay = this._cachedMetaData.Track || "";
		nickname = this._cachedMetaData.Artist || "";
		relay = Boolean(stationName)?relay:"";
		stationName = nowPlayingInfoCtrl.getStationNameText(stationName);
		if (stationName && relay)
		{
			relay = nowPlayingInfoCtrl.getRelayText(relay);
			nowPlayingInfoCtrl.setSongName(stationName);
			nowPlayingInfoCtrl.setArtistName(relay);
			nowPlayingInfoCtrl.setAlbumName(nickname);
		}
		else if (stationName)
		{
			nowPlayingInfoCtrl.setSongName(stationName);
			nowPlayingInfoCtrl.setArtistName(nickname);
		}
    }
