 ...
 */
NowPlayingInfoCtrl.prototype.getFrequencyAndUnitsText = function(frequency, units)
{
    var frequencyAndUnitText = "";

    if (frequency && units)
    {
        frequencyAndUnitText = '<span class="NowPlayingInfoCtrlFreqFragment">' + frequency + ' </span>' +
                               '<span class="NowPlayingInfoCtrlUnitFragment">' + units + '</span>';
    }
...
/*
 * Public utility function for handling Style15 radio station  station string with multiple font characteristics.
 * It builds the correct HTML snippet for styling the text fragments correctly on a single visible line.
 ...
 */
NowPlayingInfoCtrl.prototype.getStationNameText = function(station)
{
    var stationNameText = "";

    if (station)
    {
        stationNameText = '<span class="NowPlayingInfoCtrlStationFragment">' + station + '</span>';
    }
...
/*
 * Public utility function for handling Style15 radio relay station string with multiple font characteristics.
 * It builds the correct HTML snippet for styling the text fragments correctly on a single visible line.
 * NOTE: There are currently (11/20/2015) two design studio options, A and B, for handling the relay station:
 *      A: Radio station followed by relay station, in parentheses, all on one line.
 *      B: Radio station and relay station on separate lines.
 ...
 *
 * Arguments:
 *    relay: The relay station string 
 */
NowPlayingInfoCtrl.prototype.getRelayText = function(relay)
{
    var relayText = "";

    if (relay)
    {
        relayText = '<span class="NowPlayingInfoCtrlRelayFragment">(' + relay + ')</span>';
    }
...
NowPlayingInfoCtrl.prototype.dvdTvSetAtSpeed = function(atSpeed) 
{
    ...
    {
        log.error("atSpeed is null or undefined");
        return;
    }
    if(!atSpeed && (this.properties.mediaSource === "DVD" || this.properties.mediaSource === "TV")) 
    {
        log.warn("atSpeed receieved, but media source != DVD/TV, ignoring");
        return;
    }
...
{
    log.debug("Handle controller event called, ignored");
    return "ignored";
};

NowPlayingInfoCtrl.prototype.cleanUp = function() 
{
    log.debug("Clean up called, destroying NowPlayingInfoCtrl");
