...
CompassCtrl.prototype._setDirection = function (direction)
{
    ...
    if (!this._compass.hasOwnProperty(direction))
    {
        ...
        if(!cachedValidDir)
            {
                cachedValidDir = 'n';
                log.warn('cachedValidDir was NULL');
            }
    }
    else
    {
        cachedValidDir = direction;
        log.debug('_setDirection:Cached Valid direction : '+cachedValidDir);
    }

    direction = cachedValidDir;
    ...
    log.debug('_setDirection:Final Direction set on Compass: '+this._currentDir);
