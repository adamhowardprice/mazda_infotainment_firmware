...
amradioApp.prototype.appInit = function()
{
	...
	//For JAPAN Region
    this._umpButtonConfigJP = new Object();

    this._umpButtonConfigJP["SelectSource"] = this._umpButtonConfigJpn["SelectSource"];
    this._umpButtonConfigJP["SelectStationList"] =
    {
        buttonBehavior : "shortPressOnly",
        imageBase : "IcnUmpAutoMem", //changed icon to look like station list
        disabled : false,
		labelId:"ChannelListJP", //todo:need to add this
        buttonClass : "normal",
        appData : "SelectStationList"
    };
    this._umpButtonConfigJP["SelectFavoritesAudio"] = this._umpButtonConfigJpn["SelectFavoritesAudio"];
    this._umpButtonConfigJP["SelectScan"] = this._umpButtonConfigJpn["SelectScan"];
    this._umpButtonConfigJP["SelectTune"] = this._umpButtonConfigJpn["SelectTune"];
    this._umpButtonConfigJP["SelectSeekPrevious"] = this._umpButtonConfigJpn["SelectSeekPrevious"];
    this._umpButtonConfigJP["SelectSeekNext"] = this._umpButtonConfigJpn["SelectSeekNext"];
    this._umpButtonConfigJP["SelectSettings"] = this._umpButtonConfigJpn["SelectSettings"];	
    ...
    //AutoMemory Datalist
    this._AreaPresetJPDataList = {
        itemCountKnown : true,
        itemCount : 1,
        vuiSupport : true,
        items: [
            { appData : { eventName : "SelectUpdateStationList" } , text1Id : "ListUpdate", itemStyle : "style02", image1 : null , hasCaret : false }
        ]
    };
    ...
    this._contextTable = {
        ...
        "StationListJP" : {
            ...
            "controlProperties": {
               "List2Ctrl" : {
                    dataList : this._AreaPresetJPDataList,
            ...
            "leftBtnStyle": "goBack",
            "readyFunction" : this._StationListJPCtxtTmpltReadyToDisplay.bind(this),
            "contextInFunction" : this._StationListJPCtxtIn.bind(this),
            "displayedFunction": null
        },//end of AutoMemory
        ...
        "AreaPresetJP" : {
            ...
            "leftBtnStyle": "goBack",
            "readyFunction" : this._AreaPresetJPCtxtTmpltReadyToDisplay.bind(this),
            "contextInFunction" : this._AreaPresetJPCtxtIn.bind(this),
            "displayedFunction": null
        },//end of AutoMemory
...
//StationListJP Ctxt Tmplt
amradioApp.prototype._StationListJPCtxtIn = function()
{
    log.info("amradioApp _StationListJPCtxtIn called...");
    this._autoMemoryDataList = {
        itemCountKnown : true,
        itemCount : 1,
        vuiSupport : true,
...
//StationListJP Ctxt Tmplt
amradioApp.prototype._StationListJPCtxtTmpltReadyToDisplay = function()
{
    log.info("amradioApp _StationListJPCtxtTmpltReadyToDisplay called...");
	this._updateStationList = true;
	var vuiFlag = false;
...
//AreaPresetJP Ctxt Tmplt
amradioApp.prototype._AreaPresetJPCtxtIn = function()
{
    log.debug("amradioApp _AreaPresetJPCtxtIn called...");
    this._AreaPresetJPDataList = {
        itemCountKnown : true,
        itemCount : 1,
        vuiSupport : true,
...
//AreaPresetJP Ctxt Tmplt
amradioApp.prototype._AreaPresetJPCtxtTmpltReadyToDisplay = function()
{
    log.debug("amradioApp _AutoMemoryCtxtTmpltReadyToDisplay called...");
	var vuiFlag = false;
	var appData = { eventName : "UpdateTabList"};
...
amradioApp.prototype._RadioTunerStatusMsgHandler = function(msg)
{
    if (msg && msg.params && msg.params.payload && msg.params.payload.radioTunerStatus)
    {
		...
		log.info("Inside Radio Tuner Status : Operation Mode received = "+this._cachedRadioDetails.operationMode +" and Frequency received = "+this._cachedRadioDetails.freq +"and stationName received = "+this._cachedRadioDetails.stationName +" and Relay Name received = "+this._cachedRadioDetails.relayName + "and Nickname received = "+this._cachedRadioDetails.nickName);
...
amradioApp.prototype._populateScanFreqNowPlayingCtrl = function (tmplt)
{
	...
	if(tmplt && this._currentContext && this._currentContext.ctxtId === "NowPlaying")
	{
		if(this._currentOperationMode !== "AUTO_M_MODE")
		{
            ...
            {
                ctrlTitleItem = tmplt.nowPlaying4Ctrl.getStyle15FrequencyAndUnitsText(frequency,"kHz");
            }
            else
            {
        ...
        if ( focussedItem != null)
        {
			switch(eventName){
				case "SelectStationFrequency":
					...
					{payload:{radioData: { stationType : "AM", frequency : frequency, stationName : stationName, relayName : relay, PICode : piCode, HDSPS : "SPS_NONE", satRadioData : null }}});
					break;
...
amradioApp.prototype._TimedSbnStatusBarStationInfoAnalogMsgHandler = function(msg)
{
    ...
    if ( msg && msg.params && msg.params.payload)
    {
		...
		else
		{
			...
			var psnName = StationInfo.stationName ||"";
			var relay = StationInfo.relayName ||"";
			relay = Boolean(StationInfo.relayName)?relay:"";
			...
			{
				framework.common.startTimedSbn(this.uiaId, "StationsbnInfo", "typeE",
				{ sbnStyle : "Style07",imagePath1 : "common/images/icons/IcnSbnEnt.png", text1Id : "system.AmRadio" , text2 : frequency , 
					text3:"kHz",text4:psnName, text5:relay});
			}
			else
			{
			framework.common.startTimedSbn(this.uiaId, "AnalogsbnInfo", "typeE", { sbnStyle : "Style02",imagePath1 : "common/images/icons/IcnSbnEnt.png", text1Id : "system.AmRadio" , text2 : frequency}); 			
			}
...
amradioApp.prototype._StartTuneActionMsgHandler = function(msg)
{
    if (msg && msg.params && msg.params.payload)
    {
        if (this._currentContext && this._currentContextTemplate && this._currentContext.ctxtId === "Tune")
...
amradioApp.prototype._StartTuneStepChangeMsgHandler = function(msg)
{
    if (msg && msg.params && msg.params.payload)
    {
        if (this._currentContext && this._currentContextTemplate && this._currentContext.ctxtId === "Tune")
...
amradioApp.prototype._StartTuneDirectMsgHandler = function(msg)
{
    if (msg && msg.params && msg.params.payload)
    {
        if (this._currentContext && this._currentContextTemplate && this._currentContext.ctxtId === "Tune")
...
amradioApp.prototype._stopTuneTimer = function(tmplt)
{
    if(tmplt)
    {
...
amradioApp.prototype._tuneStepChange = function(tmplt,StepChange)
{
    if (StepChange)
    {
        // Tune Up
...
amradioApp.prototype._tunerDirectCtrl = function(tmplt, frequency)
{
...
amradioApp.prototype._AreaPresetListStatusInfoMsgHandler = function(msg)
{
	...
	if (msg && msg.params && msg.params.payload && msg.params.payload.AreaPresetListStatus)
	{
        ...
        if(status === "AVAILABLE")
        {
            //DBAPI interaction
            var params = {};
			framework.sendRequestToDbapi(this.uiaId, this._getAreaPresetJPCallbackFn.bind(this), "radio", "AmAreaPresetList", params);
        }
        else if (status === "FAILED")
        {
            this._populateAreaPresetFailedListCtrl();
        }
        else
        {
...
amradioApp.prototype._stepCallback = function(tunerRef, appData, params)
{
    ...
    if(params && params.direction)
    {
        ...
        if(params.stepCount)
        {
            stepParams = { payload : { "stepCount" : params.stepCount, "event_from_vui" : false}};
        }
...
amradioApp.prototype._listLongPressCallback = function(listCtrlObj, appData, params)
{
    ...
    switch(this._currentContext.ctxtId)
    {
        case "AutoMemory" :
            ...
            framework.sendEventToMmui(this.uiaId, "SelectFavoritesStation" ,
                {payload:{radioData: { stationType : "AM", frequency : frequency, stationName : stationName, relayName : null, PICode : piCode, HDSPS : "SPS_NONE", satRadioData : null }}});
            break;
...
amradioApp.prototype._listItemClickCallback = function(listCtrlObj, appData, params)
{
    ...
    switch(this._currentContext.ctxtId)
    {
		...
		case "AreaPresetJP" :
            switch(appData.eventName)
            {
                case "SelectUpdateStationList" :
                    if(this._currentContext && this._currentContextTemplate && this._currentContext.ctxtId === "StationListJP")
                    {
                        this._populateAutoMemUpdatingStationListCtrl();
                    }
					if(this._currentContext && this._currentContextTemplate && this._currentContext.ctxtId === "AreaPresetJP" )
                    {
                        this._populateAreaPresetFailedListCtrl();
                    }
                    this._cachedAutoMStatus = "UPDATING";
 ...
 */
amradioApp.prototype._tabClickCallback = function(btnRef, appData, params)
{ 
...
amradioApp.prototype._populateAMNowPlayingCtrl = function(tmplt)
{
    ...
    else if ("REGION_JPN" === this._cachedRegion)
    {
		...
		stationunit = this._currentContextTemplate.nowPlaying4Ctrl.getStyle15FrequencyAndUnitsText(frequency, "kHz"),
...
amradioApp.prototype._populateAutoMemUpdateStationListCtrl = function(tmplt)
{
    ...
    && this._cachedAutoMemoryList.autoMemoryList)
    {
		...
		else if(this._currentContext && this._currentContext.ctxtId === "StationListJP")
		{
			...
			var items = new Array();
			this._autoMemoryDataList.items[0].text1Id = "ListUpdate";
			this._autoMemoryDataList.items[0].image2 = null;
		}
        ...
        for(i = 0 ; i < autoMemoryCount ; i++)
        {
			...
			if(this._currentContext && this._currentContext.ctxtId === "StationListJP")
			{
				...
				items[i+1] =
				{
					appData : { eventName : 'SelectStationFrequency', piCode : stationPiCodeInfo , stations : frequency , stationName : stations, relayName : relay},
					text1 : frequency ,
					label1: "kHz",
					label2 : _stationName,
					itemStyle : "style38",
				...
				{
					items[i+1].image2 = "common/images/icons/IcnListEntNowPlaying_En.png";
				}				
			}
			else if(this._currentContext && this._currentContext.ctxtId === "AutoMemory")
			{
				items[i+1] =
				{
...
//AreaPreset UpdateStationList control
amradioApp.prototype._populateAreaPresetUpdateStationListCtrl = function(tmplt)
{
    var stations = null;
    var stationPiCodeInfo = null;
    var listLength = null;
    ...
    if(this._cachedAreaPresetList && this._cachedAreaPresetList.eCode == 0 && this._cachedAreaPresetList.totalCount > 0
    && this._cachedAreaPresetList.AreaPresetList)
    {
        listTotalCount = this._cachedAreaPresetList.totalCount;
        areaPresetCount = this._cachedAreaPresetList.AreaPresetList.length;

        var dataList =
            {
                itemCountKnown : true,
                itemCount : areaPresetCount + 1,
                vuiSupport : true
            };
        var items = new Array();
        this._AreaPresetJPDataList.items[0].text1Id = "ListUpdate";
        this._AreaPresetJPDataList.items[0].image2 = null;
        items = this._AreaPresetJPDataList.items;
		if(this._cachedRadioDetails)
		{
			_currFrequency = this._cachedRadioDetails.freq;
		}

        for(i = 0 ; i < areaPresetCount ; i++)
        {
            if(this._cachedAreaPresetList.AreaPresetList[i].frequency)
            {
                frequency = this._cachedAreaPresetList.AreaPresetList[i].frequency;
            }
            if(this._cachedAreaPresetList.AreaPresetList[i].piCode)
            {
                stationPiCodeInfo = this._cachedAreaPresetList.AreaPresetList[i].piCode;
            }
			
			stations = this._cachedAreaPresetList.AreaPresetList[i].stationName || "";
			relay = Boolean(this._cachedAreaPresetList.AreaPresetList[i].stationName)?this._cachedAreaPresetList.AreaPresetList[i].relayName:"";
			...
			if(_currFrequency == this._cachedAreaPresetList.AreaPresetList[i].frequency)
			{
				items[i+1].image2 = "common/images/icons/IcnListEntNowPlaying_En.png";
			}
        }
    ...
    dataList.items = items;
    this._AreaPresetJPDataList = dataList;
    tmplt.list2Ctrl.setDataList(dataList);
    tmplt.list2Ctrl.updateItems(0, dataList.itemCount - 1);
}
...
amradioApp.prototype._populateAutoMemUpdatingStationListCtrl = function()
{
    ...
    {
        this._resetStationListJPDatalist();
...
//AreaPresetJP list when Status is "FAILED"
amradioApp.prototype._populateAreaPresetFailedListCtrl = function()
{
    this._resetAreaPresetJPDatalist();
    //this._updateStationList = true; //set updateStationList to true
    this._AreaPresetJPDataList.items[0].text1Id = "ListUpdate";
	this._AreaPresetJPDataList.items[0].image2 = 'indeterminate';
    this._AreaPresetJPDataList.items[0].hasCaret = false;
    this._currentContextTemplate.list2Ctrl.setDataList(this._AreaPresetJPDataList);
    this._currentContextTemplate.list2Ctrl.focussedItem = 0;
    var length = this._AreaPresetJPDataList.itemCount;
    this._currentContextTemplate.list2Ctrl.updateItems(0, length - 1);
}
...
amradioApp.prototype._setUmpButtonConfig = function()
{
    ...
    if (this._cachedRegion)
    {
        ...
        {
            case "REGION_JPN" :
                this._buttonConfig = this._umpButtonConfigJP;
                break;			
            case "REGION_OTHER" :
...
//Resetting StationListJP
amradioApp.prototype._resetStationListJPDatalist = function()
{
    this._autoMemoryDataList = {
        itemCountKnown : true,
        itemCount : 1,
        vuiSupport : true,
        items: [
            { appData : { eventName : "SelectUpdateStationList" } , text1Id : "ListUpdate", itemStyle : "style02" , hasCaret : false }
        ]
    };
}

//Resetting Automemory
amradioApp.prototype._resetAreaPresetJPDatalist = function()
{
    this._AreaPresetJPDataList = {
        itemCountKnown : true,
        itemCount : 1,
...
amradioApp.prototype._setTunerFrequency = function(tmplt)
{
    ...
    if ( this._cachedRadioDetails && this._cachedRadioDetails.freq )
    {
        ...
        else
        {
			...
			if("REGION_JPN" === this._cachedRegion)
			{
				log.info("Recieved station name: " +this._cachedRadioDetails.stationName +"Relay Name:" + this._cachedRadioDetails.relayName);
				var units = "kHz";
				var station = "";
				var relay = "";
				station = this._cachedRadioDetails.stationName;
				relay = this._cachedRadioDetails.relayName;				
...
//AreaPreset DBAPI callback
amradioApp.prototype._getAreaPresetJPCallbackFn = function(msg)
{
    if ( msg && msg.msgContent.params )
    {
        this._cachedAreaPresetList = msg.msgContent.params;
        if(this._currentContext && this._currentContextTemplate && this._currentContext.ctxtId == "AreaPresetJP")
        {
            this._populateAreaPresetUpdateStationListCtrl(this._currentContextTemplate);
        }
    }
}
