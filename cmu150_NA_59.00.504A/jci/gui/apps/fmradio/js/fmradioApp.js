...
fmradioApp.prototype.appInit = function()
{
    ...
    //asethab
	this._cachedPresetStatus = "AVAILABLE"
    this._tempgenreList = null;
	this.tabClick = this._tabClickCallback.bind(this);
	this._userAction = false;
	this._lastViewedTab = null;
    this._cachedSubRegionJapan = "REGION_DEFAULT";
	this._stepCountData = {
		"REGION_USA" :{
									"stepInc" 			  : 0.2,
									"low"          		  : 87.7,
    ...
    //Buttons config for JAPAN region
    this._umpButtonConfigJPN = new Object();

    //Source
    this._umpButtonConfigJPN["SelectSource"] = this._umpButtonConfigUSA["SelectSource"];

    //auto memory
    this._umpButtonConfigJPN["SelectStationList"] =
    {
        buttonBehavior : "shortPressOnly",
        imageBase : "IcnUmpAutoMem", //changed icon to look like station list
        disabled : false,
		labelId : "ChannelListJP" ,  //todo:
        buttonClass : "normal",
        appData : "SelectStationList"
    };

    //Favourites
    this._umpButtonConfigJPN["SelectFavorites"] = this._umpButtonConfigUSA["SelectFavorites"];

    //Scan
    this._umpButtonConfigJPN["SelectScan"] = this._umpButtonConfigUSA["SelectScan"];

    //Tune
    this._umpButtonConfigJPN["SelectTune"] = this._umpButtonConfigUSA["SelectTune"];

    //SelectSeekPrevious
    this._umpButtonConfigJPN["SelectSeekPrevious"] = this._umpButtonConfigUSA["SelectSeekPrevious"];

    //SelectSeekNext
    this._umpButtonConfigJPN["SelectSeekNext"] =  this._umpButtonConfigUSA["SelectSeekNext"];

    //Settings
    this._umpButtonConfigJPN["SelectSettings"] =  this._umpButtonConfigUSA["SelectSettings"];
	...
	this._AreaPresetListTabCtxtDataList = {
	    itemCountKnown : true,
        itemCount : 1,
        vuiSupport : true,
		items: [
            { appData : { eventName : 'SelectUpdateStationList' }, text1Id : "ListUpdate", itemStyle : "style01", hasCaret:false}, //todo: change eventname
        ]
	...
	//Tab Config for Japan Region 
	this._tabsConfig = 
    [
        {
            "selectCallback" : this.tabClick,
			"labelId"       : "AreaPresetTab",
            "subMap"        : null,
            "tabStyle" : "tabsStyle2",
            "appData" : "SelectAreaPresetJP"
        },
        {
            "selectCallback" : this.tabClick,
            "labelId"         : "ChannelListTab",
            "subMap"        : null,
            "tabStyle" : "tabsStyle2",
            "appData" : "SelectStationListJP"
        }]
    ...
    this._contextTable = {
	    ...
	    "AreaPresetJP" : {
           "template" : "List2Tmplt",
            ...
            "controlProperties": {
                "List2Ctrl" : {
                    "dataList": this._AreaPresetListTabCtxtDataList, 
                    titleConfiguration : 'tabsTitle', 
                    tabsButtonConfig : {"style":"tabsStyle2", "defaultSelectCallback":null, "currentlySelectedTab":0, "tabsConfig": this._tabsConfig, tabsGroup : "ChannelListJP"},
					numberedList : true,
                    selectCallback : this._listItemClickCallback.bind(this),
                    longPressCallback : this._listLongPressCallback.bind(this)
                } // end of properties for "List2Ctrl"
            }, // end of list of controlProperties
              "readyFunction" : this._AreaPresetJPReadyToDisplay.bind(this),
              "contextInFunction" : this._AreaPresetJPReadyIn.bind(this)              
        }, // end of "AreaPresetJP"
		...
		"StationListJP" : {
            ...
            "controlProperties": {
                "List2Ctrl" : {
                    "dataList": this._AreaPresetListTabCtxtDataList, 
                    titleConfiguration : 'tabsTitle', 
                    tabsButtonConfig : {"style":"tabsStyle2", "defaultSelectCallback":null, "currentlySelectedTab":1, "tabsConfig": this._tabsConfig, tabsGroup : "ChannelListJP"},
					numberedList : true,
                    selectCallback : this._listItemClickCallback.bind(this),
                    longPressCallback : this._listLongPressCallback.bind(this)
                } // end of properties for "List2Ctrl"
            }, // end of list of controlProperties
            "readyFunction" : this._StationListJPReadyToDisplay.bind(this),
            "contextInFunction" : this._StationListJPReadyIn.bind(this)              
        }, // end of "StationListJP"		
    ...
    this._messageTable = {
        ...
        "RADIO_Japan_region_info" : this._RADIO_Japan_region_infoMsgHandler.bind(this),
        ...
        "StartTuneAction" : this._StartTuneActionMsgHandler.bind(this),
        "StartTuneStepChange" : this._StartTuneStepChangeMsgHandler.bind(this),
        "StartTuneDirect" : this._StartTuneDirectMsgHandler.bind(this),
		"AreaPresetListStatusInfo" : this._AreaPresetListStatusInfoMsgHandler.bind(this),
...
fmradioApp.prototype._TuneCtxtInFunction = function()
{
    ...
    switch(this._cachedRegion)
    {
        ...
        case "REGION_EU" :
            ...
            break;
        case "REGION_JPN" :
            switch(this._cachedSubRegionJapan)
            {
                case "REGION_DEFAULT":
                    region = "JA";
                    break;
                case "REGION_WIDEBAND":
                    region = "JAEXTD";
                    break;
                default :
                    region = "JA";
...
// _AutoMemoryJapanRegionCtxtInTmplt
fmradioApp.prototype._AreaPresetJPReadyIn = function()
{
    log.debug("fmradioApp _AreaPresetJPReadyIn called...");

};

fmradioApp.prototype._StationListJPReadyIn = function()
{
    log.debug("fmradioApp _StationListJPReadyIn called...");
    this._fmradioMemoryDataList = {
        itemCountKnown : true,
        itemCount : 1,
        vuiSupport : true,
        items: [
            { appData : {eventName : 'SelectUpdateStationList' }, text1Id : "ListUpdate", itemStyle : "style01", hasCaret:false},
        ]
    };
};
...
//AutoMemoryJapan Region Ctxt Tmplt Ready to Display
fmradioApp.prototype._AreaPresetJPReadyToDisplay = function()
{
    log.debug("Value of this._cachedPresetStatus is = "+this._cachedPresetStatus +" , Value of this._currentOperationMode is = "+this._currentOperationMode);
	var vuiFlag = false;
	var appData = { eventName : "UpdateTabList"};
	framework.sendEventToMmui(this.uiaId, appData.eventName, vuiFlag);
		this._populateAreaPresetFailedStationListCtrl();
};

fmradioApp.prototype._StationListJPReadyToDisplay = function()
{
    log.info("Value of this._cachedAutoMStatus is = "+this._cachedAutoMStatus +" , Value of this._currentOperationMode is = "+this._currentOperationMode);
	var vuiFlag = false;
	var appData = { eventName : "UpdateTabList"};
...
fmradioApp.prototype._TimedSbn_StatusBarStationInfoMsgHandler = function(msg)
{
    if(msg && msg.params && msg.params.payload)
    {
		...
		relay = msg.params.payload.relayName || "";
        ...
        this._showFrequencySBN(frequency,picode,subStationPlaying,psnName,relay);
    }
};
...
fmradioApp.prototype._StartTuneActionMsgHandler = function(msg)
{
    if (msg && msg.params && msg.params.payload)
    {
        if (this._currentContext && this._currentContextTemplate && this._currentContext.ctxtId === "Tune")
        {
            this._stopTuneTimer(this._currentContextTemplate);
        }
    }
}

fmradioApp.prototype._StartTuneStepChangeMsgHandler = function(msg)
{
    if (msg && msg.params && msg.params.payload)
    {
        if (this._currentContext && this._currentContextTemplate && this._currentContext.ctxtId === "Tune")
        {
            this._stopTuneTimer(this._currentContextTemplate);
            this._tuneStepChange(this._currentContextTemplate,msg.params.payload.TuneStepChange );
        }
    }
}

fmradioApp.prototype._StartTuneDirectMsgHandler = function(msg)
{
    if (msg && msg.params && msg.params.payload)
    {
        if (this._currentContext && this._currentContextTemplate && this._currentContext.ctxtId === "Tune")
        {
            this._stopTuneTimer(this._currentContextTemplate);
            var frequency = msg.params.payload.radioTunerStatus.freq;

            this._tunerDirectCtrl(this._currentContextTemplate, frequency );
        }
    }
}

fmradioApp.prototype._stopTuneTimer = function(tmplt)
{
    if(tmplt)
    {
        tmplt.tuner4Ctrl.stopSettleTimer();
    }
};

fmradioApp.prototype._tuneStepChange = function(tmplt,StepChange)
{
    if (StepChange)
    {
        // Tune Up
        tmplt.tuner4Ctrl._tickUp();
    }
    else
    {
        // Tune Down
        tmplt.tuner4Ctrl._tickDown();
    }
    tmplt.tuner4Ctrl._startSettleTimer();
}

fmradioApp.prototype._tunerDirectCtrl = function(tmplt, frequency)
{
...
fmradioApp.prototype._AreaPresetListStatusInfoMsgHandler = function(msg)
{
	if (msg && msg.params && msg.params.payload && msg.params.payload.AreaPresetListStatus)
	{
        var status = msg.params.payload.AreaPresetListStatus;
        if(status === "AVAILABLE")
        {
            //DBAPI interaction
            var params = {};
			framework.sendRequestToDbapi(this.uiaId, this._getAreaPresetListJPCallbackFn.bind(this), "radio", "FmAreaPresetList", params);
        }
        ...
        this._cachedPresetStatus = status;
	}
}
...
fmradioApp.prototype._RadioTunerStatusMsgHandler = function(msg)
{
    if (msg && msg.params && msg.params.payload && msg.params.payload.radioTunerStatus)
    {
		...
		log.info("Inside Radio Tuner Status : Operation Mode received = "+this._cachedRadioDetails.operationMode +" and Frequency received ="+this._cachedRadioDetails.freq +" and Station Name received ="+this._cachedRadioDetails.stationName +" and Relay Name received ="+this._cachedRadioDetails.relayName +" and Nickname received ="+this._cachedRadioDetails.nickName);
		...
		else if("REGION_JPN" === this._cachedRegion)
		{
			stepCount = this._stepCountData[this._cachedRegion][this._cachedSubRegionJapan].stepInc;
		}
		else
		{
...
fmradioApp.prototype._RegionInformationMsgHandler = function(msg)
{
    if (msg && msg.params && msg.params.payload)
    {
        ...
        if((this._cachedRegionInfo.region && this._cachedRegionInfo.region !== this._cachedRegion && this._cachedRegionInfo.region !== "REGION_4A") &&
            (this._cachedRegionInfo.region && this._cachedRegionInfo.region !== this._cachedRegion && this._cachedRegionInfo.region !== "REGION_JPN"))//If region is 4A or Japan then wait for sub-region 
        {
...
fmradioApp.prototype._SelectFocusedStationAsFavoriteMsgHandler = function(msg)
{
    if (this._currentContextTemplate && this._currentContextTemplate.list2Ctrl)
    {
        ...
        if(stationList.items[focussedItem] && stationList.items[focussedItem].appData)
        {
            ...
            if((stationList.items[focussedItem].appData.relayName) && ("REGION_JPN" === this._cachedRegion))
            {
               relay = stationList.items[focussedItem].appData.relayName;
            }
        }
        ...
        if ( focussedItem != null )
        {
			...
			case "SelectStationName":
				framework.sendEventToMmui(this.uiaId, "SelectFavoritesStation" ,
					{payload:{radioData: { stationType : "FM", frequency : frequency, stationName : stationName, relayName : relay, PICode : piCode, HDSPS : "SPS_NONE", satRadioData : null }}},true);
				break;
...
//Sub region info for Japan Region message received from MMUI
fmradioApp.prototype._RADIO_Japan_region_infoMsgHandler = function(msg)
{
    if (msg && msg.params && msg.params.payload && msg.params.payload.subRegionJapanInfo)
    {
        if(msg.params.payload.subRegionJapanInfo !== this._cachedSubRegionJapan)
        {
            this._cachedSubRegionJapan = msg.params.payload.subRegionJapanInfo;
            //set the button config in nowplaying according to the region received from MMUI
            this._setButtonConfig();
        }
    }
...
fmradioApp.prototype._stepCallback = function(tunerRef, appData, params)
{
    ...
    if(params && params.direction)
    {
        ...
        {
            stepParams = { payload : { "stepCount" : params.stepCount , "HD_ON" : false, "FMHDSpsNo":"SPS_NONE","FMFrequency": frequency, "event_from_vui" : false}};
 ...
 */
fmradioApp.prototype._tabClickCallback = function(btnRef, appData, params)
{ 
    log.info(" _tabClickCallback  called...", appData + "Last viewed Tab" + this._lastViewedTab); 
	if (appData !== this._lastViewedTab)
    {
        framework.sendEventToMmui(this.uiaId, appData, { payload: {} });
		this._lastViewedTab = appData;
    }
...
fmradioApp.prototype._listItemClickCallback = function(listCtrlObj, appData, params)
{
    ...
    switch(this._currentContext.ctxtId)
    {
        ...
        case "AutoMemory" :
            ...
            else if (appData.eventName === "UpdateAutoMemory" || appData.eventName === "SelectUpdateStationList")
            {
                if(this._currentContext && this._currentContextTemplate && (this._currentContext.ctxtId === "AutoMemory" || this._currentContext.ctxtId === "StationListJP"))
                {
                    this._populateAutoMemUpdatingStationListCtrl();
                }
				else if(this._currentContext && this._currentContextTemplate && this._currentContext.ctxtId === "AreaPresetJP")
				{
				 this._populateAreaPresetFailedStationListCtrl();
				}
...
fmradioApp.prototype._listLongPressCallback = function(listCtrlObj, appData, params)
{
    ...
    switch(this._currentContext.ctxtId)
    {
        ...
        case "AutoMemory" :
            ...
            framework.sendEventToMmui(this.uiaId, "SelectFavoritesStation" ,
                {payload:{radioData: { stationType : "FM", frequency : frequency, stationName : stationName, relayName : null, PICode : piCode, HDSPS : "SPS_NONE", satRadioData : null }}});
            break;
        ...
        case "StationListJP" :
            ...
            var stationName = appData.stationName;
            var frequency = appData.frequency;
            var relay = appData.relayName;
            framework.sendEventToMmui(this.uiaId, "SelectFavoritesStation" ,
                {payload:{radioData: { stationType : "FM", frequency : frequency, stationName : stationName, relayName : relay, PICode : piCode, HDSPS : "SPS_NONE", satRadioData : null }}});
            break;
        default :
            break;
    }
...
fmradioApp.prototype._populateTunerCtrl = function(tmplt)
{
    ...
    if(this._cachedRadioDetails && this._cachedRadioDetails.freq)
    {
        ...
        else
        {
			...
			{
				tmplt.tuner4Ctrl.setFrequency(frequency,hdConfigObj);
				if("REGION_JPN" === this._cachedRegion)
				{
					var units = "MHz";
					var station = this._cachedRadioDetails.stationName || "";
					var relay = this._cachedRadioDetails.relayName || "";
					
					tmplt.tuner4Ctrl.setUnitStationRelay(frequency,units,station,relay);   
				}
			}            
        }
    }
    ...
    else
    {
        ...
        switch(currentRegion)
        {
            ...
            case "REGION_JPN" :
				...
				if(this._cachedRadioDetails)
				{
					nickname = this._cachedRadioDetails.nickName || "";
					relay = this._cachedRadioDetails.relayName || "";				
				}
				relay = Boolean(stationName)?relay:"";
				stationunit = this._currentContextTemplate.nowPlaying4Ctrl.getStyle15FrequencyAndUnitsText(frequency, "MHz"),
				station = this._currentContextTemplate.nowPlaying4Ctrl.getStyle15StationAndRelayText(stationName, relay);
...
fmradioApp.prototype._populateAutoMemUpdateStationListCtrl = function(tmplt)
{
	...
	var relayName = null;
	var _stationName = null;
	var _currFrequency = null;
    ...
    if(this._cachedAutoMemoryList && this._cachedAutoMemoryList.eCode == 0 && this._cachedAutoMemoryList.totalCount > 0)
    {
		...
		if(this._currentContext && this._currentContext.ctxtId === "AutoMemory")
		{
			var dataList =
			{
				itemCountKnown : true,
		...
		else if (this._currentContext && this._currentContext.ctxtId === "StationListJP")
		{
			
			if(this._cachedRadioDetails)
			{
				_currFrequency = this._cachedRadioDetails.freq;
			}
			
			var dataList =
			{
				itemCountKnown : true,
				itemCount : listTotalCount + 1,
				vuiSupport : true
			};
			var items = new Array();
			this._fmradioMemoryDataList.items[0].text1Id = "ListUpdate";
			this._fmradioMemoryDataList.items[0].itemStyle = "style01";
		}
        ...
        for(var i = 0 ; i < listTotalCount ; i++)
        {
			...
			if(this._currentContext && this._currentContext.ctxtId === "StationListJP")
			{

				stations = this._cachedAutoMemoryList.autoMemoryList[i].stationName || "";
				relay = Boolean(this._cachedAutoMemoryList.autoMemoryList[i].stationName)?this._cachedAutoMemoryList.autoMemoryList[i].relayName:"";
				_stationName = this._currentContextTemplate.list2Ctrl.getStationAndRelay(stations,relay);
				
				items[i + this._fmradioMemoryDataList.itemCount] =
				{
					appData : { eventName : 'SelectStationName', piCode : stationPiCodeInfo , frequency : frequencyAppData , stationName : stations, relayName : relay},
					text1 : frequency,
					label1 : "MHz",
					label2 : _stationName,
					itemStyle : "style38",
				...
				if(_currFrequency == this._cachedAutoMemoryList.autoMemoryList[i].frequency)
				{
					items[i + this._fmradioMemoryDataList.itemCount].image2 = "common/images/icons/IcnListEntNowPlaying_En.png";
				}
			}
			else
			{
	...
	else if(tmplt && tmplt.list2Ctrl && this._currentContext && this._currentContext.ctxtId === "StationListJP")
    {
        tmplt.list2Ctrl.setDataList(dataList);
        tmplt.list2Ctrl.updateItems(0, dataList.itemCount - 1);
    }
...
//ChannelList UpdateStationList control
fmradioApp.prototype._populateAreaPresetListCtrl = function(tmplt)
{
    var stations = null;
    var stationPiCodeInfo = null;
    var listTotalCount = null;
    ...
    if(this._cachedChannelList && this._cachedChannelList.eCode == 0 && this._cachedChannelList.totalCount > 0)
    {
		if(this._cachedRadioDetails)
		{
			_currFrequency = this._cachedRadioDetails.freq;
        ...
        listTotalCount = this._cachedChannelList.totalCount;
        var dataList =
        {
            itemCountKnown : true,
            itemCount : listTotalCount + 1,
        ...
        var items = new Array();
        this._AreaPresetListTabCtxtDataList.items[0].text1Id = "ListUpdate";
        this._AreaPresetListTabCtxtDataList.items[0].itemStyle = "style01";
		this._AreaPresetListTabCtxtDataList.itemCount = 1;
        items = this._AreaPresetListTabCtxtDataList.items;

        for(var i = 0 ; i < listTotalCount ; i++)
        {
            if(this._cachedChannelList.AreaPresetList[i].frequency)
            {
                frequency = this._cachedChannelList.AreaPresetList[i].frequency;
                frequencyAppData = frequency;
                frequency = this._frequencyFormatting(frequency);
            }
            if(this._cachedChannelList.AreaPresetList[i].piCode)
            {
                stationPiCodeInfo = this._cachedChannelList.AreaPresetList[i].piCode;
            }

			stations = this._cachedChannelList.AreaPresetList[i].stationName || "";
			relay = Boolean(this._cachedChannelList.AreaPresetList[i].stationName)?this._cachedChannelList.AreaPresetList[i].relayName:"";
			
			
			_stationName = this._currentContextTemplate.list2Ctrl.getStationAndRelay(stations,relay);
			
            items[i + this._AreaPresetListTabCtxtDataList.itemCount] =
            {
                appData : { eventName : 'SelectStationName', piCode : stationPiCodeInfo , frequency : frequencyAppData , stationName : stations, relayName : relay},
                text1 : frequency,
				label1 : "MHz",
			...
			if(_currFrequency == this._cachedChannelList.AreaPresetList[i].frequency)
			{
				items[i + this._AreaPresetListTabCtxtDataList.itemCount].image2 = "common/images/icons/IcnListEntNowPlaying_En.png";
			}
        }
    }
    //If data is empty or error in fetching data then display "No results found"
    ...
    dataList.items = items;
    this._AreaPresetListTabCtxtDataList = dataList;
    if(tmplt && tmplt.list2Ctrl && this._currentContext && this._currentContext.ctxtId === "AreaPresetJP")
    {
        tmplt.list2Ctrl.setDataList(dataList);
        tmplt.list2Ctrl.updateItems(0, dataList.itemCount - 1);
    }
...
fmradioApp.prototype._populateAutoMemUpdatingStationListCtrl = function()
{
	...
	if("REGION_JPN" === this._cachedRegion)// If the region is JPN then then need to show "List Upadate Item only"
    {
        this._resetingStationListJPCtxDataList();
    }
    this._updateAutoMRequest  = true;
    ...
    else if(this._currentContextTemplate && this._currentContextTemplate.list2Ctrl && this._currentContext && this._currentContext.ctxtId === "StationListJP")
    {
        this._currentContextTemplate.list2Ctrl.setDataList(this._fmradioMemoryDataList);
        this._currentContextTemplate.list2Ctrl.focussedItem = 0;
        var length = this._fmradioMemoryDataList.itemCount;
...
//AutoMemory displays updating List with progress bar
fmradioApp.prototype._populateAreaPresetFailedStationListCtrl = function()
{
    this._resetingAreaPreasetJPCtxDataList();

    this._updateAutoMRequest  = true;
    if(this._currentContextTemplate && this._currentContextTemplate.list2Ctrl && this._currentContext && this._currentContext.ctxtId === "AreaPresetJP")
    {
        this._currentContextTemplate.list2Ctrl.setDataList(this._AreaPresetListTabCtxtDataList);
        this._currentContextTemplate.list2Ctrl.focussedItem = 0;
        var length = this._AreaPresetListTabCtxtDataList.itemCount;
        this._currentContextTemplate.list2Ctrl.updateItems(0, length - 1);
    }
};
...
fmradioApp.prototype._setButtonConfig = function()
{
    ...
    switch(this._cachedRegion)
    {
        case "REGION_OTHER" :
            ...
            break;		
        case "REGION_JPN" :
            this._UmpButtonConfig = this._umpButtonConfigJPN;
            this._region = "Other";
			this._FreqSeekTimer = 20;
            break;
        case "REGION_USA" :
...
//Reseting the datalist of AutoMemory Context
fmradioApp.prototype._resetingStationListJPCtxDataList = function()
{
    //@formatter:off
    this._fmradioMemoryDataList = {
        itemCountKnown : true,
        ...
        vuiSupport : true,
        items: [
            { appData : {eventName : 'SelectUpdateStationList' }, text1Id : "ListUpdate", itemStyle : "style02", hasCaret:false,image2:'indeterminate'}
        ]
    };
};

//Reseting the datalist of AutoMemory Context
fmradioApp.prototype._resetingAreaPreasetJPCtxDataList = function()
{
    //@formatter:off
    this._AreaPresetListTabCtxtDataList = {
        itemCountKnown : true,
...
//Helper function  for AutoMemory DBAPI
fmradioApp.prototype._getAreaPresetListJPCallbackFn = function(msg)
{
    //AutoMemory DBAPI callback
    if ( msg && msg.msgContent.params )
    {
        this._cachedChannelList = msg.msgContent.params;
        this._populateAreaPresetListCtrl(this._currentContextTemplate);
    }
};

//Helper function  for  Japan region AutoMemory DBAPI
fmradioApp.prototype._getJapanAutoMemoryCallbackFn = function(msg)
{
    //AutoMemory DBAPI callback
    if ( msg && msg.msgContent.params )
    {
...
fmradioApp.prototype._showFrequencySBN = function(frequency,picode,subPlayingStation,psnName,relay)
{
    if(frequency && frequency !== 0)
    {
		...
		else
		{
			...
			if(this._cachedRegion === "REGION_JPN")
			{
				framework.common.startTimedSbn(this.uiaId, "StationsbnInfo", "typeE",
				{ sbnStyle : "Style07",imagePath1 : "common/images/icons/IcnSbnEnt.png", text1Id : "system.FmRadio" , text2 : sbnText , 
					text3:"MHz",text4:psnName, text5:relay});
			}			
			else
			{
				framework.common.startTimedSbn(this.uiaId, "StationsbnInfo", "typeE",
				{ sbnStyle : "Style02",imagePath1 : "common/images/icons/IcnSbnEnt.png", text1Id : "system.FmRadio" , text2 : sbnText });
			}			
		}
...
fmradioApp.prototype._populateScanFreqNowPlayingCtrl = function (tmplt)
{
	...
	if(tmplt && this._currentContext && (this._currentContext.ctxtId === "NowPlaying" || this._currentContext.ctxtId === "GenreSeek"))
	{
		...
		{ 
            if ("REGION_JPN" === this._cachedRegion)
            {
                ctrlTitleItem = tmplt.nowPlaying4Ctrl.getStyle15FrequencyAndUnitsText(frequency,"MHz");
            }
            else
            {
...
fmradioApp.prototype._populateScanFreq = function (tmplt)
{
	...
	switch(this._currentOperationMode)
	{
		...
		case "SEEK_UP_MODE":
			...
			{
				if(disPlayedFreq ==  this._stepCountData[this._cachedRegion][this._cachedSubRegionJapan].high)
				{
					freqToDisplay = this._stepCountData[this._cachedRegion][this._cachedSubRegionJapan].low;
				}
				else
				{
					freqToDisplay = this._displayedFreq + this._stepCount;
		...
		case "SEEK_DOWN_MODE":
			...
			{
				if(disPlayedFreq ==  this._stepCountData[this._cachedRegion][this._cachedSubRegionJapan].low)
				{
					freqToDisplay = this._stepCountData[this._cachedRegion][this._cachedSubRegionJapan].high;
				}
				else
				{
					freqToDisplay = this._displayedFreq - this._stepCount;
