...
btaudioApp.prototype.appInit = function()
{
    ...
    this._umpButtonConfigA2DP["Play/Stop"] =
    {
        buttonBehavior : "shortPressOnly",
        ...
        stateArray: [
            ...
            {
                state:"Stop", label:null, labelId:null
            },
...
{
    if(msg && msg.params && msg.params.payload && 0 <= msg.params.payload.time)
    {
...
btaudioApp.prototype._FileNavigationNeedDataCallback = function(index, params)
{
    ...
    if(this._cachedFolderItem !== null)
    {
        log.debug('cachedFolderItem.index : ' + this._cachedFolderItem.index);
        if(isNaN(this._cachedFolderItem.index) === false && index > this._cachedFolderItem.index)
        {
            index = this._cachedFolderItem.index;
        }
    }
    ...
    if((index > this._offset && index <((this._requestChunkSize+this._offset)-10)) || index == 0 || this._isDataRequesting === true || this._subItemCount <= 25)
    {
...
{
	log.info(" PlayerState : "+this._playerState);
	if(this._currentContext.ctxtId === "NowPlayingA2DP")
	{
		log.info(" updating for NowPlayingA2DP"); // added for NowPlayingA2DP
		if (this._playerState === "PLAYING" || this._playerState === "SCAN_FWD" ||    this._playerState === "SCAN_BKD")
		{
			state = "Stop";
			template.nowPlaying4Ctrl.umpCtrl.setButtonState("Play/Stop", state);
		}
		else
		{
			state = "Play";
