...
function GuiFramework()
{
    ...
    if (window["guiManager"])
    {
        ...
        this._heartbeatMonitor.overridePluginWatchdogValue(45); //this will set the opera heartbeat timeout to 45 sec (SW00157253)
...
GuiFramework.prototype._getStartupSettings = function()
{
    ...
    setTimeout( function()
    {
        if (!self.initGuiCalled)
        {
            ...
            {
                log.debug("SYS_SETTINGS app didn\'t set region, using Region_NorthAmerica");
            ...
            {
                log.debug("SYS_SETTINGS app didn\'t set ditance unit, using Miles");
            ...
            {
                log.debug("SYS_SETTINGS app didn\'t set VehicleType, using J36");
...
GuiFramework.prototype.routeMmuiMsg = function(jsObject)
{
        ...
        if (jsObject.msgType === "transition" && jsObject.enabled === false)
        {
            ...
            while (index >= 0)
            {
                if (this._msgQueue[index].msg.msgType === "transition" && this._msgQueue[index].msg.enabled === false)
                {
                    ...
                    log.debug("Removed an existing transition false from msgQueue before appending a new transition false message.");
                }
                --index;
            }
        }
    ...
    {
        log.debug("GUI cannot send a message to an App called 'unknown'. Ignoring message: " + JSON.stringify(jsObject));
        return;
    }
        ...
        case "focusStack":
            ...
            else
            {
                ...
                for (var i = 0; i < jsObject.appIdList.length; i++)
                {
                    ...
                    {
                        log.debug("GUI cannot send a message to an App called 'unknown'. Ignoring message: " + jsObject);
                        return;
                    }
                }
            }
...
GuiFramework.prototype.routeDbapiMsg = function(jsObject)
{
    ...
    if (jsObject.msgType == "methodResponse")
    {
        ...
        if (this._dbapiReqCallbacks[callbackFnKey])
        {
            ...
            {
                log.warn("Matching App callback for DBAPI response not found for ", callbackFnKey);
            }
        }
    }
    else
...
GuiFramework.prototype.routeAppsdkMsg = function(jsObject)
{
    ...
    if (jsObject.msgType === "methodResponse" || jsObject.msgType === "methodErrorResponse")
    {
        ...
        if (this._appsdkReqCallbacks[callbackFnKey])
        {
            ...
            {
                log.warn("Matching App callback for APPSDK response not found for ", callbackFnKey);
            }
        }
    }
    else
            ...
            if (appChanged || instance.hasContextChanged())
            {
                if (this._currTmpltInstance)
                {
                    ...
                    if (instance.isContextDifferentFrom(this._currTmpltInstance.contextInfo) == false)
                    {
                        ...
                        log.debug("Framework thinks we're in:", JSON.stringify(this._currTmpltInstance.contextInfo), "and App thinks we're in", JSON.stringify(instance.getCurrentContext()));
                        return;
                    }
                }
                ...
                else
                {
                    ...
                    this._showTemplateSurfaces(this._currTmpltInstance, prevTmpltInstance);
 ...
 */
GuiFramework.prototype._showTemplateSurfaces = function(someTemplate, prevTemplate)
{
    ...
    if (someTemplate.contextInfo.uiaId !== "backupparking" && prevTemplate && prevTemplate.contextInfo.uiaId !== "backupparking")
    {
...
{
    log.debug("_createLoadingScreen has been deprecated in favor of a full loading context in the syssettings application.");
...
GuiFramework.prototype._setLanguageSpecificStyling = function(languageCode)
{
    ...
    {
        log.debug("Right-to-Left Language detected. Changing styling to match.");
//        document.body.style.direction = "rtl";
    ...
    else
    {
        ...
        log.debug("Left-to-Right Language detected. Changing styling to match.");
...
GuiFramework.prototype._addTmpltFiles = function(templateName, path)
{
    ...
    else
    {
        ...
        {
            log.debug("Template name and template path do not match. Possible typo in context table.");
        }
    }
            ...
            for (i; i < this._msgQueue.length; i++)
            {
                if (i < this._msgQueueYieldPoint)
                {
                    ...
                    switch (this._msgQueue[i].socketType)
                    {
                        ...
                        default:
                            log.debug("Invalid socket type message present in GUI Busy queue")
                            break;
                    }
                }
                else
...
{
    log.error("State Timeout Error: Framework never returned to IDLE state from: " + this._frameworkState + " Before the error, GUI was in App:"+this._currentAppUiaId + " and context: " + this.getCurrCtxtId());
    log.writeError(err);
    ...
    if (queue && queue.length > 0)
    {
        ...
        {
            log.debug("    Message queue contains: ", queue);
        }
        else
        {
    ...
    switch (this._frameworkState)
    {
        ...
        case this._FWK_STATE_LOADING_CMN_GLOBAL_CTRLS:
            ...
            {
                log.warn("Framework has timed out: Files were loaded, but some error occurred while trying to start the GUI Common Module. The GUI may not have started.");
            }
            break;
        case this._FWK_STATE_LOADING_APP:
            if (someFileFailed)
            ...
            {
                log.warn("Framework has timed out: App files loaded, but some other error occurred while attempting to initialize the App: " + this._lastAppLoaded);
            }
            break;
        case this._FWK_STATE_LOADING_APP_CTRLS:
            if (someFileFailed)
            ...
            {
                log.warn("Framework has timed out: Control files loaded, but some other error occurred while attempting to instantiate the Control.");
            }
            break;
        case this._FWK_STATE_TMPLT_CHANGE:
    ...
    if (this.debugMode)
    {
        log.debug("Framework has timed out while in debug mode. Attempting to return to IDLE state.");
...
GuiFramework.prototype.setSharedData = function(uiaId, name, value)
{
    ...
    {
        log.debug("setSharedData called with invalid uiaId or name.");
    }
}
...
GuiFramework.prototype.getSharedData = function(uiaId, name)
{
    ...
    {
        log.debug("getSharedData called with invalid uiaId or name.");
    }
}
...
GuiFramework.prototype.subscribeToSharedData = function(uiaId, sharedDataUiaId, sharedDataName)
{
    ...
    {
        log.debug("subscribeToSharedData called with invalid uiaId or sharedDataUiaId or sharedDataName");
    }
}
