...
function Localization()
{
    ...
    this.LANG_CODES = {
		...
		"LANGS_KO_KOREAN"      : "en_US",               /* US (North American) English for Korean*/
    ...
    this._appVrHelpDicts = new Object(); // (Object) holds all currently loaded dictionary objects for VrHelp
    // Initialize _prevAppVrHelpDicts to null, rather than new Object(), to avoid dictionary misses when changing language
    // to the same language & language change fails.  Also matches postcondition for successful language changes.
    this._prevAppVrHelpDicts = null;
...
{
    log.debug("setLang() FUNCTION DEPRECATED USE setlanguage()"); 
...
Localization.prototype.unloadDictionaries = function(langCode, status)
{
    ...
    {
        if (this._prevAppDicts == null && this._prevAppVrHelpDicts == null)
        {
    ...
    else if (langCode == this._currentLang && (!status))
    {
        ...
        log.warn("Language change failed. Reverting to previous language styling: " + this._prevLang);
        ...
        this._appVrHelpDicts = this._prevAppVrHelpDicts;
        this._prevAppDicts = null; // null all at once
        this._prevAppVrHelpDicts = null;
...
Localization.prototype.loadAppDict = function(uiaId)
{
    ...
    var appVrHelpDictName = "__VrHelpPositionTable__" + uiaId + "AppDict_" + this._currentLang; //for VrHelp Strings Lookup
    ...
    if (window[appVrHelpDictName])
    {
        // get the Object from the window space
        var dictVrHelp = window[appVrHelpDictName];
        // Store the new language's dictionary
        this._appVrHelpDicts[uiaId] = dictVrHelp;
    }
};
...
/*
 * Test whether the currently loaded dictionary for an app contains the __VrHelpPositionTable__XXX.
 * @return true if the __VrHelpPositionTable__XXX is present for the given app or false if not.
 */
Localization.prototype.testVrHelpPosTableExistence = function(defaultAppName, helpPromptName)
{
    var uiaId = defaultAppName;
    if (defaultAppName && helpPromptName)
    {
        if (this._appVrHelpDicts[uiaId] && this._appVrHelpDicts[uiaId][helpPromptName])
        {
            return true;
        }
    }
...
/*
 * Returns the found stringID from the VrHelp position table (lookup)
 * This API is to be used by GUI Common. In general, it should NOT be called by an App.
 *
 * @param   defaultAppName  (string) The uiaId of the app the control is in
 * @param   helpPromptName  (string) The name of the help prompt to look for in the lookup table
 * @param   position        (string) The position of the localized text in the given help prompt (e.g. "Right" or "Left")
 * @param   indx            (int) The index for the prompt text (e.g. Left[1,2,..,5] or Right[1,2,..,5])
 * @return  (string)        The found stringID. If a stringID cannot be found (or is not loaded) for the given values, the null string will be returned
 *
 */
Localization.prototype.getVrHelpStringId = function(defaultAppName, helpPromptName, position, indx)
{
    var uiaId = defaultAppName;
    var stringID = null
    var index =  indx - 1; // as index ranges from (0-4) whereas 'indx' varies from (1-5)
    if (defaultAppName && helpPromptName)
    {
       if(this._appVrHelpDicts[uiaId][helpPromptName][position])
        {
            stringID = this._appVrHelpDicts[uiaId][helpPromptName][position][index];
...
Localization.prototype.getLocStr = function(defaultAppName, stringId, subMap)
{
    ...
    if (defaultAppName && stringId)
    {
        ...
        if (this._appDicts[uiaId])
        {
            if (this._appDicts[uiaId].hasOwnProperty(stringId))
            {
                ...
                if (subMap)
                {
                    ...
                    {
                        log.warn("Invalid subMap passed with stringId: " + stringId + ". SubMaps must be of type Object.");
                    }
                }

                if (localStr.length === 0)
                {
                    log.warn("App dictionary miss (" + this._currentLang + "): " + defaultAppName + " requested '" + originalStringId + "' but the translated string is blank in the dictionary.");
                    localStr = stringId;
            ...
            {
                log.warn("App dictionary miss (" + this._currentLang + "): " + defaultAppName + " requested '" + originalStringId + "' but that stringId is not found.");
                localStr = stringId;
                if (guiConfig.showStringIdFailure)
                {
                   localStr = this._currentLang + ":" + stringId;
        ...
        {
            log.warn("App dictionary miss (" + this._currentLang + "): " + defaultAppName + " requested '" + originalStringId + "' but that translation file is not loaded. Does the file exist?");
            localStr = stringId;
            if (guiConfig.showStringIdFailure)
            {
               localStr = this._currentLang + ":" + stringId;
...
{
    log.debug("getCurrLang() FUNCTION DEPRECATED USE getCurrentlanguage()"); 
...
Localization.prototype.setLanguage = function(enumKey, vrSupport, reloadAll, callbackFn)
{
    ...
    { 
        ...
        this._prevAppVrHelpDicts = utility.deepCopy(this._appVrHelpDicts);
...
{
    log.debug("setDateFormat() FUNCTION DEPRECATED");
}
...
{
    log.debug("getDateFormat() FUNCTION DEPRECATED");
}
