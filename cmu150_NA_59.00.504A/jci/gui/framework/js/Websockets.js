...
function Websockets()
{
    ...
    { 
        ...
        setTimeout(function()
        {
            log.debug("Websockets not available in debug mode. Now testing offline.");
            this._offline = true;
...
Websockets.prototype._openMmui = function()
{
    ...
    this._mmuiWs.onclose = function(e)
    {
        if (this._fwkMmuiCallbackSent)
        {
            log.warn("MMUI websocket closed or could not be opened with e.wasClean: "+e.wasClean + "  e.reason: "+e.reason + "  e.code: "+e.code);
...
Websockets.prototype._openAppSdk = function()
{
    ...
    {
        log.debug("APPSDK websocket open");
    }
    ...
    this._appWs.onclose = function(e)
    {
        if (this._fwkAppSdkCallbackSent)
        {   
            log.warn("APPSDK websocket closed or could not be opened with e.wasClean: "+e.wasClean+"  e.reason:"+e.reason+"  e.code:"+e.code);
...
Websockets.prototype._openDbapi = function()
{
    ...
    {
        log.debug("DBAPI websocket open");
    }
    ...
    this._dbapiWs.onclose = function(e)
    {
        if (this._fwkDbApiCallbackSent)
        {
            log.warn("DBAPI webscoket closed or could not be opened with e.wasClean: "+e.wasClean+"  e.reason: "+e.reason+"  e.code: "+e.code);
