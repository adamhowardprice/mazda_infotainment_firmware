...
function Common()
{
    ...
    this._vuiExamplesDialog = null; // (Object) The active VUI Examples Dialog Control (null if none displayed)
    ...
    this._lastDisplayStateSent = 0; // (Number) saves last display off notification state
    ...
    this._messageTable = {
        ...
        "Global.StartExamples" : this._ShowVuiExamplesHandler.bind(this),
        "Global.HideExamples" : this._HideVuiExamplesHandler.bind(this),
...
Common.prototype.addControls = function()
{
    ...
    else
    {
        ...
        var statusProp = {
            ...
            "onFocusCallback" : this._statusBarFocused.bind(this),
        };
...
Common.prototype.getCommonTransitionData = function(template)
{
    ...
    this._closeVuiExamplesOnTransition(template);
    ...
    if ((this._remoteUiBtns[this._cachedTemplate.contextInfo.uiaId])&& (this._remoteUiBtns[this._cachedTemplate.contextInfo.uiaId].ctxtList.indexOf(this._cachedTemplate.contextInfo.ctxtId) != -1))
    {
        this.statusBar.showRemoteUiButton(this._remoteUiBtns[this._cachedTemplate.contextInfo.uiaId]);
    }
    else
    {
        this.statusBar.hideRemoteUiButton();
    }
    ...
    if (this._canShowSbns)
    {
        ...
        if (!this._displayedSbnId)
        {
            ...
            if (newSbnId)
            {
                ...
                this._sendDisplayOffNotification(1);
            }
...
{
    var response = null;
    if (this.statusBar.remoteUiBtn) // the events will be passed only when the carPlay button exists
    {
        if (this.statusBar.remoteUiBtn.hasFocus)
        {
            response = this.statusBar.handleControllerEvent("lostFocus");
        }
    }
...
Common.prototype.handleControllerEvent = function(eventId, firstEvent)
{
    ...
    if (this._cachedTemplate && this._cachedTemplate.handleControllerEvent)
    {
        ...
        if (eventId != "touchActive" && eventId != "controllerActive")
        {
            ...
            else if (this.statusBar.remoteUiBtn && this.statusBar.remoteUiBtn.hasFocus)
            {
                // route events to Status Bar
                var response = this.statusBar.handleControllerEvent(eventId);
                if (response == "giveFocusDown")
                {
                    var response = this._cachedTemplate.handleControllerEvent("acceptFocusFromTop");
                    if (response == "consumed")
                    {
                        this.statusBar.handleControllerEvent("lostFocus");
                    }
                }
            }
            else
            {
                ...
                else if (response == "giveFocusUp" && this.statusBar.remoteUiBtn)
                {
                    this.statusBar.remoteUiBtn.handleControllerEvent("acceptFocusFromBottom");
                    this._cachedTemplate.handleControllerEvent("lostFocus");
                }
            }
        }
        else
        {
            ...
            if (this.statusBar)
            {
                this.statusBar.handleControllerEvent(eventId);
            }
...
Common.prototype._ShowVuiHelpHandler = function(msg)
{
    ...
    // If examples prompt is there when help is called, then removes examples prompt first (functions check for the validity of prompt internally)
    this._HideVuiExamplesHandler(null);
...
Common.prototype._getVuiHelpDialogConfiguration = function(uiaId, helpPromptName)
{
    ...
    // __VrHelpPositionTable__ flag for the current app 
    var vrHelpPosTable = framework.localize.testVrHelpPosTableExistence(uiaId, helpPromptName);
    var leftStringId = null;
    var rightStringId = null;
    ...
    for (var i = 1; i <= 5; ++i)
    {
        ...
        {
            // new approach
            leftStringId = framework.localize.getVrHelpStringId(uiaId, helpPromptName,"Left", i);
            rightStringId = framework.localize.getVrHelpStringId(uiaId, helpPromptName,"Right", i);    
        }
        else
        {
            // old approach TODO: to be removed when all the VR apps supports the new VrHelpPositionTable. 
            leftStringId = "VrHelp_" + helpPromptName + "_Left" + i;
            rightStringId = "VrHelp_" + helpPromptName + "_Right" + i;
        }
        
        if (leftStringId && framework.localize.testLocStr(uiaId, leftStringId))
        {
            config.dialogConf["text" + i.toString() + "Id"] = leftStringId;
            found = true;
        }
        if (rightStringId && framework.localize.testLocStr(uiaId, rightStringId))
        {
...
/*
 *  Handler for Global.StartExamples message
 * @tparam  Object  msg     The parsed message object sent from Mmui
 */
Common.prototype._ShowVuiExamplesHandler = function(msg)
{
    log.debug("Common.prototype._ShowVuiExamplesHandler called", msg);

    // TODO: check requirements and confirm.
    // If help prompt is there when examples is called, then removes help prompt first (functions check for the validity of prompt internally)
    this._HideVuiHelpHandler(null);

    // Hide any currently displayed VUI Examples
    this._HideVuiExamplesHandler(null);
    ...
    if (framework.getCurrentApp() === msg.params.payload.currentUiaId &&
        framework.getCurrCtxtId() === msg.params.payload.currentContextId)
    {
        // Try to load a VUI examples configuration and display it if found
        var targetUiaId = msg.params.payload.currentUiaId;
        var examplesId = msg.params.payload.examplesId;
        var dialogProperties = this._getVuiExamplesDialogConfiguration(this.moduleName, examplesId);
        if (dialogProperties)
        {
            this._vuiExamplesDialog = framework.instantiateControl(dialogProperties.uiaId, document.body, 'Dialog3Ctrl', dialogProperties.dialogConf);
            this._vuiExamplesDialog.divElt.classList.add("Dialog3Ctrl_Style19_NoTemplateInstance");

            // Add properties to the control to record the uiaId and contextId. See also _closeVuiExamplesOnTransition().
            this._vuiExamplesDialog._uiaId = msg.params.payload.currentUiaId;
            this._vuiExamplesDialog._contextId = msg.params.payload.currentContextId;
        }
    }
    else
    {
        log.warn("Ignoring Global.StartExample message because GUI is in context '"
            + framework.getCurrentApp()
            + " "
            + framework.getCurrCtxtId()
            + "' but the message is for a different context: "
...
/*
 * Handler for Global.HideExamples message
 * @tparam  Object  msg     (optional) The parsed message object sent from Mmui
 */
Common.prototype._HideVuiExamplesHandler = function(msg)
{
    log.debug("Common.prototype._HideVuiExamplesHandler called");

    if (this._vuiExamplesDialog)
    {
        framework.destroyControl(this._vuiExamplesDialog);
        this._vuiExamplesDialog = null;
    }
};

/*
 * Close Vui example at the start of a transition to a new template if that template represents a different context.
 */
Common.prototype._closeVuiExamplesOnTransition = function(template)
{
    if (this._vuiExamplesDialog
        && template
        && template.contextInfo.uiaId != this._vuiExamplesDialog._uiaId
        && template.contextInfo.ctxtId != this._vuiExamplesDialog._contextId)
    {
        this._HideVuiExamplesHandler(null);
    }
};
...
/*
 * Returns a configuration object for a VUI examples dialog3 control prompt from an app's dictionary.
 * Returns null if no prompt text is found in the dictionary.
 * @tparam String uiaId The UIA or Module to use in the Dialog Configuration.
 * @tparam Number examplesId Id of the example prompt to look for in the dictionary.
 */
Common.prototype._getVuiExamplesDialogConfiguration = function(uiaId, examplesId)
{
    var config = {
        dialogConf: {
            contentStyle: "style26",
            titleStyle : "titleStyle03"
        },
        uiaId: uiaId
    };
    // TODO: check if we have any requirement for defualt example screen
    // if so add default configuration same as helpPrompt
    if(examplesId < 1 || examplesId > 28)
    {
        log.error("ExamplesId " + examplesId + " is not supported. Support range is 1-28.");
    }
    else
    {
        if (guiResources.VR_EXAMPLES["examplesId"+ examplesId])
        {
            // get textIds
            for (var num = 1; num <= 5; ++num)
            {
                var StringId = guiResources.VR_EXAMPLES["examplesId"+examplesId]["textId"+num];
                if(StringId && framework.localize.testLocStr(uiaId, StringId))
                {
                    config.dialogConf["text" + num.toString() + "Id"] = StringId;
                }
                else
                {
                    config.dialogConf["text" + num.toString() + "Id"] = "";
                }
            }

            // get title
            var titleId = guiResources.VR_EXAMPLES["examplesId"+ examplesId].titleText;
            if (titleId && framework.localize.testLocStr(uiaId, titleId))
            {
                config.dialogConf.titleId = titleId;
            }
            else
            {
                config.dialogConf.titleId = "";
            }

        }
        else
        {
            log.error("ExamplesId does not exist in guiResources.VR_EXAMPLES table");
        }
    }
    return config ;
};
...
Common.prototype._displaySbnFromQueue = function()
{
    ...
    if (this._sbnQueue.length == 0)
    {
        ...
        this._sendDisplayOffNotification(0);
        return newSbnToDisplay;
    }
...
Common.prototype._statusBarClicked = function(ctrlObj, appData, params)
{
    ...
    if (params && params.statusBtn == "home")
    {
    ...
    else if (params && params.statusBtn == "carPlay")
    {
        if (typeof this._remoteUiBtns[this._cachedTemplate.contextInfo.uiaId].selectCallback == 'function')
        {
            this._remoteUiBtns[this._cachedTemplate.contextInfo.uiaId].selectCallback(ctrlObj, appData, params);
        }
    }
...
/*
 * Callback for when the Status Bar is focused
 * @param   ctrlObj (Object) Reference to the button control object in status bar that was clicked
 * @param   appData (Object) Data passed in by the app when the control was instantiated
 * @param   params  (Object) Optional params passed by the control object
 */
Common.prototype._statusBarFocused = function(ctrlObj, appData, params)
{
    log.debug("status bar focused", ctrlObj, appData, params);
    // being used and called only for carplay for now
    if (params && params.statusBtn == "carPlay")
    {
        if (typeof this._remoteUiBtns[this._cachedTemplate.contextInfo.uiaId].onFocusCallback == 'function')
        {
            this._remoteUiBtns[this._cachedTemplate.contextInfo.uiaId].onFocusCallback(ctrlObj, appData, params);
        }
    }
...
Common.prototype.getContextCapture = function()
{
    return {
        ...
        statusBarHasFocus: this.statusBar.remoteUiBtn && this.statusBar.remoteUiBtn.hasFocus
    };
};
...
Common.prototype.restoreContext = function(restoreData)
{
    ...
    else if (restoreData.commonContextCapture && restoreData.commonContextCapture.statusBarHasFocus)
    {
        if (this.statusBar.remoteUiBtn)
        {
            this.statusBar.remoteUiBtn.handleControllerEvent('acceptFocusInit');
        }
    }
};
...
Common.prototype.setRemoteUiButton = function(uiaId,enabled,config)
{
    if(enabled)
    {
        if (this._cachedTemplate)
        {
            this._remoteUiBtns[uiaId] = config ;
            if ((this._remoteUiBtns[this._cachedTemplate.contextInfo.uiaId])&& (this._remoteUiBtns[uiaId].ctxtList.indexOf(this._cachedTemplate.contextInfo.ctxtId) != -1))
            {
                this.statusBar.showRemoteUiButton(this._remoteUiBtns[uiaId]);
            }
            else
            {
                log.info("not on home screen so not showing the carPlay icon");
            }
        }
    }
    else
    {
         this._remoteUiBtns[uiaId] = null ;
         this.statusBar.hideRemoteUiButton();
    }
};
...
/*
 * Sends display off notification event to MMUI
 * @param sendDisplayOn (Number) 1 or 0 indicating if display should be on or off respectively.
 */
Common.prototype._sendDisplayOffNotification = function(sendDisplayOn)
{
    if (sendDisplayOn != this._lastDisplayStateSent)
    {
        framework.sendEventToMmui("system", "DisplayOffNotificationEvent", {"payload": {"notificationActive": sendDisplayOn}});
        this._lastDisplayStateSent = sendDisplayOn;
    }
};
