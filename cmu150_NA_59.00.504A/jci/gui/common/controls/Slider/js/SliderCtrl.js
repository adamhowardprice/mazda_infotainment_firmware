...
function SliderCtrl(uiaId, parentDiv, controlId, properties)
{
    ...
    this._darMoveMax = null;            // (Number) Maximum value for dar bar
    this._darMoveMin = null;            // (Number) Minimum value of dar bar
    ...
    //@formatter:off
    this.properties = {
        "style": "slider",              // (String) "pivot", "slider", "progress", "dar"
    ...
    if (this.properties.style == 'dar')
    {
        this._currSliderIndex = 1;
    }
...
SliderCtrl.prototype._createStructure = function()
{
    ...
    if(this.properties.style == 'dar')
    {
        ...
        this.dar = document.createElement('div');
        this.dar.className = this.properties.darClass;
        this.divElt.appendChild(this.dar);
    }
    //scrubber bar
	...
	if(this.properties.style != "dar")
	{
		// Set slider to inital value
		 this._setValue(this.properties.value);
	}
...
SliderCtrl.prototype.setMoveIncrement = function(min, max, increment)
{
    if (this.properties.style == 'dar')
    {
        log.error("SliderCtrl: setMoveIncrement() API is CAN NOT be used for this style ", this.properties.style);
        return;
    }
...
SliderCtrl.prototype.setValue = function(value)
{
    if (this.properties.style == 'dar')
    {
        log.error("SliderCtrl: _setValue() API is deprecated for this style ", this.properties.style);
        return;
    }
...
// internal private function. helper to public setValue() API
SliderCtrl.prototype._setValue = function(value)
{
    log.debug("SliderCtrl: _setValue() ", value);
 ...
 */
SliderCtrl.prototype.setScrubberValues = function(min, max, value)
{
    log.info("SliderCtrl: setScrubberValues() ", min, max, value);
    if (this.properties.style == 'dar')
    {
        if (min != null)
    	{
            this._darMoveMax = 1-min;     //setting right boundary for playhead
        }
        if (max != null)
        {
            //if max is less than current slider index(reverse) then set max to current slider index
            if ((value == null) && (max < 1 - this._currSliderIndex))
            {
                max = 1 - this._currSliderIndex;
            }
            this._darMoveMin = 1-max;     //setting left boundary for playhead
            var xPerc = 100*max;
            this.dar.style.width = xPerc * this._sliderWidth / 100 + "px";
            this.dar.style.left = (this._sliderWidth -  xPerc * this._sliderWidth / 100) + 1 + "px";   //one pixel added so that slider background  and dar fill do not overlap
        }
        if (value != null)
        {
            //setting red bar width(Fill from right)
 ...
 * the _setValue() API or upon the settleTimer completing.
 */
...
SliderCtrl.prototype._getIndexSnapValue = function(value)
{
    ...
    if(this.properties.style != 'dar') //check that snap is within [min, max], otherwise set to min/max
    {
    ...
    {
    	if (snap <= this._darMoveMin)
        {
            return this._darMoveMin;
        }
        else if (snap >= this._darMoveMax)
        {
            return this._darMoveMax;
        }
    }
    return snap;
};
...
{
	if (this._moveMax - this._moveMin != 0)
	{
    	return this._toFixedDecimals( (index - this._moveMin)/(this._moveMax - this._moveMin) );
    }
    else
    {
    	return this._toFixedDecimals(index - this._moveMin);
    }
};
...
SliderCtrl.prototype._moveHandle = function(percent)
{
    ...
    if(this.properties != null)
    {
        ...
        else if (this.properties.style == 'dar')
        {
            this.fill.style.width = this._sliderWidth - (xPerc * this._sliderWidth / 100) + "px";
    	    this.fill.style.left = xPerc * this._sliderWidth / 100 + 1 + "px";    //one pixel added so that slider background  and fill div do not overlap
        }
        else
        {
...
SliderCtrl.prototype._onDownHandler = function(evt)
{
    ...
    if (this._selectMode === null)
    {
        if (this._isDisabled === false)
        {
            ...
            if (this.properties.style == "dar" && relativeX < Number(this.dar.offsetLeft))
            {
                return;
            }
            ...
            var sliderPercentage = this._toFixedDecimals(relativeX / this._sliderWidth);
...
SliderCtrl.prototype._onMoveHandler = function(evt)
{
    ...
    if (this.properties.style == 'dar')
	{
		...
		sliderPercentage =  this._indexToPercentage(newIndex);
	}
    this._moveHandle(sliderPercentage);
    this._slideCallbackHelper();
};
