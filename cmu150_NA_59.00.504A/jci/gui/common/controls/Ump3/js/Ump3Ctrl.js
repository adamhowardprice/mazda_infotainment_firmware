...
v2.1 (3-June-2014)
        - Added new api setUmpScrubberConfig() to set alternate scrubber style
        - restructured code to have different function creating scrubber and ump controls
        - Added ne apis _createScrubber(), _createUmpButtons and _removeScrubber()
        __________________________________________________________________________
...
function Ump3Ctrl(uiaId, parentDiv, controlId, properties)
{
    ...
    this._elapsedLabelSetAllowed = true;        // (Boolean) Used only for dar style elapsed label (set only by setSliderValues())
...
Ump3Ctrl.prototype._createStructure = function()
{
    ...
    this. _createScrubber();
    this._createUmpButtons();  
};

Ump3Ctrl.prototype._createScrubber = function()
{
    ...
    if(this.properties.hasScrubber && !this._umpScrubber)
    {
        ...
        switch(scrubberStyle)
        {
            ...
            case "Style03":
                ...
                break;
            case "Style06":
                sliderProperties.width = 580; // Actual 578
            default:
                break;
        }
        ...
        if (scrubberStyle == "Style06")
        {
            // forcing mode to be "dar" for style06 even if app ignores
            sliderProperties.mode = "dar";
            sliderProperties.darClass = "Ump3CtrlDar" + scrubberStyle;
            sliderProperties.disabledDarClass = "Ump3CtrlDarDisabled" + scrubberStyle;
        }
        ...
        else
        {
            ...
            else if (sliderProperties.mode == "dar")
            {
            	sliderProperties.style = "dar";
            }
            else
            {
    ...
    if(this.properties.hasScrubber)
    {
        ...
        if(this.properties.scrubberConfig.scrubberStyle == "Style01" || this.properties.scrubberConfig.scrubberStyle == "Style06")
        {
            // If Style01 or Style06, both elapsed and total time shown
            if(!this._elapsedTimeLabel)
            {
                this._elapsedTimeLabel = document.createElement('div');
                this._elapsedTimeLabel.className = "Ump3CtrlElapsedTime";
            ...
            if (this.properties.scrubberConfig.scrubberStyle == "Style06")
            {
                //set scrubber values in case of style06
                this.setScrubberValues(this.properties.scrubberConfig.min, this.properties.scrubberConfig.max, this.properties.scrubberConfig.value);
            }
        }
...
Ump3Ctrl.prototype._createUmpButtons = function()
{
...
/*
 * Sets scrubber dynamically according to scrubberConfig'.
 * @param scrubberConfig Object   Scrubber configuration from app
 */

Ump3Ctrl.prototype.setUmpScrubberConfig = function(scrubberConfig)
{    
    if (scrubberConfig)
    {   
        //allowed only for scrubber type 'dar' and 'slider'     
        if ((scrubberConfig.mode == 'slider') || (scrubberConfig.mode == 'dar'))
        {
            this._removeScrubber();
            this.properties["scrubberConfig"] = scrubberConfig;
            this._createScrubber();
        }
    }
...
Ump3Ctrl.prototype.setElapsedTime = function(elapsedTime)
{
    ...
    {
        // for style6, right hand label (total time) is elapsed time
        if (this.properties.scrubberConfig.scrubberStyle == "Style06")
        {
            if (this._elapsedLabelSetAllowed)
            {
            ...
            {
                log.warn("Ump3Ctrl: setElapsedTime() NOT allowed. Playing LIVE. IGNORING ");
            }
        }
        else
        {
...
Ump3Ctrl.prototype.setTotalTime = function(totalTime)
{
    ...
    //for style6, left hand label (elapsed time) is Total time
    else if(this.properties.scrubberConfig.scrubberStyle == "Style06")
    {
    	this._elapsedTime = totalTime;
        this._elapsedTimeLabel.innerText = this._elapsedTime;
    }
    else
    {
        log.warn(this.uiaId + ": Ump3Ctrl setTotalTime() is NOT supported for style " + this.properties.scrubberConfig.scrubberStyle);
    }
};
...
/*
 * Sets slider values in style 'dar'.
 * @param min   Number  Minimum value on the playhead
 * @param max   Number  Maximum value on the dar scale
 * @param value Number  current playhed
 */
Ump3Ctrl.prototype.setScrubberValues = function(min, max, value)
{
    // for now only allowed for style06
    if (this.properties.scrubberConfig.scrubberStyle == "Style06")
    {
        log.debug("Ump3Ctrl: setScrubberValues() ", min, max, value);
        if (!isNaN(value))
        {
            if (value == 0)
            {
                this.setElapsedTime(framework.localize.getLocStr("common", 'LIVE', null));
                this._elapsedLabelSetAllowed = false;
            }
            else
            {
                this._elapsedLabelSetAllowed = true;
            }
        }
        // just pass on to slider. Slider takes care of default parameter logic
        this._umpScrubber.setScrubberValues(min, max, value);
    }
    else
    {
        log.warn("Ump3Ctrl: setScrubberValues() is allowed only for style06. IGNORING. Requested for " + this.properties.scrubberConfig.scrubberStyle);
    }
};
...
Ump3Ctrl.prototype._removeScrubber = function()
{
    log.debug("Ump3Ctrl: _removeScrubber() called..."); 
    framework.destroyControl(this["_umpScrubber"]);
    this._umpScrubber = null;
};
