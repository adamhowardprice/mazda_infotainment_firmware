...
function Tuner4Ctrl(uiaId, parentDiv, controlId, properties)
{
    ...
    this._dialMouseChangeBinder = this._dialMouseChange.bind(this);
    this._redirectButtonEventBinder = this._redirectButtonEventHandler.bind(this);
...
{ 
    ...
    // Button(Next and Previous) background
    this._buttonMask = document.createElement('div');
        this._buttonMask.className = "Tuner4CtrlButtonMask";
        this._buttonMask.addEventListener("mousedown", this._redirectButtonEventBinder, false);
        this._buttonMask.addEventListener("mousemove", this._redirectButtonEventBinder, false);
        this._buttonMask.addEventListener("mouseout", this._redirectButtonEventBinder, false);
        this._buttonMask.addEventListener("mouseover", this._redirectButtonEventBinder, false);
        this._buttonMask.addEventListener("mouseup", this._redirectButtonEventBinder, false);
        this.divElt.appendChild(this._buttonMask);
...
*/
    ...
    this._needle = document.createElement('img');
        ...
        this._units.className = "Tuner4CtrlUnits";
        this.divElt.appendChild(this._units);
        
        // set station and relay for Ja-fm
        this._stationAndRelay = document.createElement('div');
        this._stationAndRelay.className = "Tuner4CtrlStationRelay";
        this.divElt.appendChild(this._stationAndRelay);
...
*/
    ...
    this._selectionMask = document.createElement('div');
        ...
        this._selectionMask.addEventListener("mousedown", this._dialMouseChangeBinder, false);
        this._selectionMask.addEventListener("mousemove", this._dialMouseChangeBinder, false);
...
Tuner4Ctrl.prototype._setFrequency = function(freq, hdConfigObj)
{
    ...
    var inputFeqencyIsEqual = false;
    if (this._curFreq && this._curFreq.kHz == freq.kHz)
    {
        inputFeqencyIsEqual = true;
    }

    if (this._canSetAppFreq || inputFeqencyIsEqual)
    {
...
// Handles touch/mouse down/move events on the dial.
Tuner4Ctrl.prototype._dialMouseChange = function(evt)
{
    ...
    {
        this._dialArea = true;
        // if we're on the circular part as well on the underlying button control
        if (evt.type == "mousemove")
        {
            if (this._mouseDown)
            {
                this._redirectMouseChangeEventHandler(evt);
            }
        }
        else
        {
    ...
    else if (dx2dy2 > this._dialData.outerRadiusSquared)
    {
        ...
        {
            this._redirectButtonEventHandler(evt);
        }
        else
        {
            this._redirectMouseChangeEventHandler(evt);
...
// Event handler to redirect mouse events(mouseover and mouseout)
Tuner4Ctrl.prototype._redirectMouseChangeEventHandler = function(evt)
{
    // Is "mousedown" event already occurred?
    if (this._mouseDown)
    {
        if (((evt.clientY - 60) <= this._dialData["buttonBottomY"]) &&
            (Math.abs(evt.clientX - 400) <= this._dialData["buttonWidth"])
            && (evt.type != "mouseout") && (!this._dialArea))
        {
            // We are still within the button area and mousedown already occurred, so button should be remain selected
            this._dispatchButtonEvent("mouseover", evt);
        }
        else
        {
            // Either we mouseover the button area or we mouseout the control area. In both case, it should behave as "mouseout" event.
            this._dispatchButtonEvent("mouseout", evt);
        }
    }
}

// Event handler attached to the button background(next and previous) to redirect mouse events
Tuner4Ctrl.prototype._redirectButtonEventHandler = function(evt)
{
    if (evt.type == "mousedown")
    {
        if (((evt.clientY - 60) <= this._dialData["buttonBottomY"]) &&
                ...
                && (!this._dialArea))
        {
            //Setting mouseDown to True
            this._mouseDown = true;
            this._dispatchButtonEvent(evt.type, evt);
        }
    }
    else
    {
        ...
        {
            if (this._mouseDown && evt.type == "mouseup")
            {
                //Setting mouseDown to False
                this._mouseDown = false;
            }
            this._dispatchButtonEvent(evt.type, evt);
        }
...
// Event handler attached to the Button(next and previous) background Div to redirect mouse events
Tuner4Ctrl.prototype._dispatchButtonEvent = function(evtName, evt)
{
    if (evt.initMouseEvent)
    {
        var newEvt = this._mouseEvent(evtName, evt.screenX, evt.screenY, evt.clientX, evt.clientY);
        var targetDiv;
...
Tuner4Ctrl.prototype.stopSettleTimer = function()
{
    // Prevent setFrequency during holds of tune button
    this._canSetAppFreq = true;
    clearTimeout(this._pendingSetFrequencyTimeoutId);
...
/*
 * API for setting the unit , station and unit for ja-fm
 */
Tuner4Ctrl.prototype.setUnitStationRelay = function(freq,unit,station, relay)
{
    var stationRelay = "";
    for (var i = 0; i <= this._lastFreqBandIdx; i++) 
    {
    ...
    if (curBandData.name=="ja-fm" || curBandData.name=="jaextd-fm" || curBandData.name=="ja-am") 
    {
        if(freq)
        { 
            this._units.innerText = unit;   
            if(station && relay)
            {
                stationRelay = station+" ("+relay +")"; 
                this._stationAndRelay.innerText = stationRelay;         
            }
            else if(station&&(relay==""||relay==null))
            {
            ...
            {
                //If No station then relay name should also not be displayed
                this._stationAndRelay.innerText = "";
            }
        }
        else
        {
            //don't show unit, station and relay if freq is not avalable
            this._units.innerText = "";
            this._stationAndRelay.innerText = "";
        };
