...
function KeyboardCtrl(uiaId, parentDiv, ctrlId, properties)
{
    ...
    this._layouts = {
        ...
        'sv_SE' : [
            {name:'lettersSmall', style:'sixTwelve', spaceText: "mellanslag", buttons:[
                [ '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', '+', '' ],
                [ 'q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p', 'å', '' ],
        ...
        'el_GR_ABC' : [
            {name:'lettersSmall', style:'fiveTen', spaceText: "διάστημα", buttons:[
                [ 'α', 'β', 'γ', 'δ', 'ε', 'ζ', 'η', 'θ', 'ι', 'κ' ],
                [ 'λ', 'μ', 'ν', 'ξ', 'ο', 'π', 'ρ', 'σ', 'τ' ],
                [ 'υ', 'φ', 'χ', 'ψ', 'ω', '',  '' ]
            ], layoutText: '123'},
...
KeyboardCtrl.prototype._updateSuggestions = function()
{
    ...
    if (this._predEngine)
    {
        if (this.properties.locale == "ja_JP_ABC" & (this._candidateZero.length < 18) )
        {
            var da = this._candidateZero;
            var url = "http://localhost:8080/?"+da;

            this.httpEeq.open('GET', url, false);

            this.httpEeq.send("");

            // Communication end event check
            if(this.httpEeq.readyState == 4) {
                _newSuggestions = decodeURI(this.httpEeq.responseText).split(",");
            } 
            else
            {
                _newSuggestions = this._candidateZero;
            }
        }
        else
        {
        ...
        log.debug("Key: ", this._candidateZero, "   count: ", _newSuggestions.length, "   value: ", _newSuggestions); // Debug_info

        //FIXME: remove this once implemented in the plugin
        if (_newSuggestions.length > 300)
        {
            _newSuggestions = _newSuggestions.slice(0, 300); //grab 300 elements in the array starting at index 0.
        }
...
// Http
    ...
    if (e.target.id.indexOf('suggestion') >= 0) //make sure 'suggestion' is in ID
    {
        if (this._predEngine)
        {
            if (this.properties.locale == "ja_JP_ABC")
            {
                var eda = e.target.dataset.value;
                var url = "http://localhost:8080/?/"+eda;
                ...
                // Communication end event check
                if(this.httpEeq.readyState == 4) {
                    log.debug("httpEeq.open select ok", e.target.dataset.value);
                } 
                else
                {
                    log.info("httpEeq.open select error", e.target.dataset.value);
                }
            }
        }
    }
    ...
    if (typeof this.properties.okBtnCallback == 'function')
    {
        ...
        if (this._predEngine)
        {
            if (this.properties.locale == "ja_JP_ABC")
            {
                ...
                // Communication end event check
                if(this.httpEeq.readyState == 4) {
                    log.debug("httpEeq.open enter ok");
                } 
                else
                {
                    log.info("httpEeq.open enter error");
                }
            }
        }
    }
...
//        this._predEngine.LoadDictionary('JAP');
        this._isJaJpConversionLoaded = true;
        this.httpEeq = new XMLHttpRequest();
    ...
    {
        this._predEngine = new PredictionEngine();
        this._predEngine.LoadDictionary('CN_TC');
        this._isZhTwConversionLoaded = true;
            ...
            if (pressedKey.dataset.enabled === "true" || (pressedKey === this._btnBackspace.divElt && this._btnBackspace.isEnabled === true))    //Disabled check
            {
                            ...
                            (this._isJaJpConversionLoaded || this._isZhCnConversionLoaded || this._isZhTwConversionLoaded || this._isCnHkConversionLoaded || this._isMnTwConversionLoaded))
                {
                    ...
                    if (this._predEngine)
                    {
                        if (this.properties.locale == "ja_JP_ABC")
                        {
                            var eda = this._focusedKey.dataset.value;
                            var url = "http://localhost:8080/?/"+eda;
                            ...
                            // Communication end event check
                            if(this.httpEeq.readyState == 4) {
//                                log.info("httpEeq.open select ok", this._focusedKey.dataset.value);
                            } 
                            else
                            {
                                log.info("httpEeq.open select error", this._focusedKey.dataset.value);
                            }
                        }
                    }
